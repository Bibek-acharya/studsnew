...
export default function MessageBubble({ message, isOwn, ownInitials, otherInitials, onEdit, onDelete }: MessageBubbleProps) {
  ...
  if (isOwn) {
    return (
      <div className="flex justify-end items-end space-x-2 my-2.5 group">
        <div className="max-w-[80%] sm:max-w-[70%] flex flex-col items-end relative group/action">
          ...
          <div className={`relative bg-blue-600 text-white p-3.5 rounded-2xl shadow-sm text-xs leading-relaxed ${isDeleted ? "opacity-60" : ""}`}>
            {isDeleted ? (
              ...
              <>
                ...
                {isLong && (
                  <button
                    ...
                    className="text-blue-200 text-[10px] mt-1 underline"
                  >
                      ...
                      <div key={attachment.id}>
                        {attachment.file_type.startsWith("image/") ? (
                          ...
                          <a
                            ...
                            className="flex items-center gap-1.5 text-blue-200 hover:text-white underline"
                          >
                ...
                <div className="flex items-center justify-end space-x-1 mt-1 text-[9px] text-white/80 font-medium">
  ...
  return (
    <div className="flex justify-start items-end space-x-2 my-2.5">
      {avatarEl}
      <div className="max-w-[80%] sm:max-w-[70%] flex flex-col items-start relative">
        <div className="flex items-center space-x-1.5 mb-1 text-[10px] text-slate-400">
          <span>{timeStr}</span>
          {message.edited_at && <span>(edited)</span>}
        </div>
        <div className={`relative bg-white text-slate-800 p-3.5 rounded-2xl border border-slate-200 shadow-sm text-xs leading-relaxed ${isDeleted ? "opacity-60" : ""}`}>
          {isDeleted ? (
                    ...
                    <div key={attachment.id}>
                      {attachment.file_type.startsWith("image/") ? (
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-1.5 text-blue-600 hover:text-blue-700 underline"
                        >
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                          </svg>
