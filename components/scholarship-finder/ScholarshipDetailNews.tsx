...
export default function NewsTab({ scholarship }: { scholarship: any }) {
      ...
      items.push(
        ...providerNews
          ...
          .map((n: any) => ({
            ...
            link: n.id ? `/news/${n.slug || `provider-${n.id}`}` : "#",
  ...
  const icons = [
    <FileText size={80} className="text-white/90" />,
    <CheckCircle size={80} className="text-white/90" />,
    <Users size={80} className="text-white/90" />,
    <Calendar size={80} className="text-white/90" />,
  ];
  return (
    <div>
      <div className="mb-6">
              ...
              >
                <div className="p-4 pb-0">
                  {item.image_url ? (
                    ...
                    <div
                      className={`flex h-40 items-center justify-center rounded-md bg-gradient-to-br ${gradients[i % gradients.length]} overflow-hidden`}
                    >
