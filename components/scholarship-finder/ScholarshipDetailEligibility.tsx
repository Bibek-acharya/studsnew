...
export function EligibilityTab({
  ...
  criteria: string[];
  docs: string[];
  selectionSteps: { num: string; title: string; desc: string }[] | null;
  sectionTitle?: string;
  sectionSubtitle?: string;
}) {
  if (criteria.length === 0 && docs.length === 0 && !selectionSteps)
    return null;
  return (
    <div>
      <div className="mb-6">
      ...
      <div className="space-y-6">
        ...
        {selectionSteps && <SelectionProcessSteps steps={selectionSteps} />}
        {docs.length > 0 && (
          <div className="rounded-md border border-amber-200 bg-amber-50 p-6">
            <h3 className="mb-3 flex items-center gap-2 text-[16px] font-bold text-amber-900">
              <CircleAlert size={20} className="text-amber-600" /> Required
              Documents
            </h3>
...
export function SelectionProcessSteps({
  steps,
}: {
  steps: { num: string; title: string; desc: string }[] | null;
}) {
    ...
    <div className="rounded-md border border-gray-100 bg-white p-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        {steps.map((s, i) => (
          <div
            key={s.num}
            className={`rounded-md p-4 text-center ${bgs[i % bgs.length]}`}
          >
            <div
              className={`mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full ${colors[i % colors.length]} font-bold text-white`}
            >
              {s.num}
            </div>
            <h4
