...
export default function OpsPanel({ from, to }: { from: string; to: string }) {
  ...
  return (
    <Panel title="Ops" onExport={() => downloadCSV(`ops-${from}-${to}.csv`, seriesToCSV(ops.series))}>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Tile label="Forum reports" value={ops.totals.forum_reports} />
        <Tile label="Review reports" value={ops.totals.review_reports} />
        <Tile label="Feedback" value={ops.totals.feedback} />
          ...
          <table className="mt-2 w-full text-sm text-gray-700">
            ...
            <tbody>
              {ops.recent_broadcasts.map((b) => (
                <tr key={b.id}>
                  <td className="py-1">Broadcast #{b.id}</td>
