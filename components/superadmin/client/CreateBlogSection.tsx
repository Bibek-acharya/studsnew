...
import { superadminBlogApi } from "@/services/superadminBlogApi";
...
export default function CreateBlogSection({ setActiveSection, editId }: { setActiveSection: (s: string) => void; editId?: number }) {
  ...
  useEffect(() => {
    if (editId != null) {
      (async () => {
        try {
          const blog = await superadminBlogApi.getById(editId);
          setTitle(blog.title || "");
  ...
  const handleImageSelect = useCallback(async (file: File) => {
    ...
    setError("");
    try {
      const url = await superadminBlogApi.uploadImage(file);
      setImageUrl(url);
      setImagePreview(url);
    } catch (err: any) {
      setImageUrl("");
  ...
  const handleSave = useCallback(async (asDraft: boolean) => {
    ...
    try {
      ...
      if (isEditing && editId) {
        await superadminBlogApi.update(editId, payload);
        toast.success(asDraft ? "Blog updated as draft." : "Blog updated successfully.");
      } else {
        await superadminBlogApi.create(payload);
        toast.success(asDraft ? "Blog saved as draft." : "Blog published successfully.");
      }
      setActiveSection("manage-blog");
    } catch (err: any) {
      setError(err.message || "Failed to save blog");
    } finally {
      setSubmitting(false);
    }
  }, [title, category, content, excerpt, imageUrl, author, tags, isEditing, editId, setActiveSection]);
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="text" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-500" placeholder="Author name" value={author} onChange={(e) => setAuthor(e.target.value)} />
            </div>
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Short Description / Excerpt</label>
          ...
          <div>
            ...
            <input type="text" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-500" placeholder="education, scholarship, nepal (comma separated)" value={tags} onChange={(e) => setTags(e.target.value)} />
          </div>
        </div>
      </div>
