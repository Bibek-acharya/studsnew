...
export default function SuperadminCreateAdmissionSection({
  setActiveSection,
}: {
  ...
  const editId =
    typeof window !== "undefined"
      ? localStorage.getItem("superadmin_edit_admission_id")
      : null;
  ...
  useEffect(() => {
    ...
    const fetchAdmission = async () => {
      try {
        const res = await superadminAdmissionApi.getById(Number(editId));
        if (res?.data) {
          const d =
              ...
              <div className="grid grid-cols-5 gap-3">
                {heroBanners.map((url, idx) => (
                  <div
                    key={idx}
                    onClick={() =>
                      document.getElementById(`sa-banner-input-${idx}`)?.click()
                    }
                ...
                {heroBanners.length < 5 && (
                  <div
                    onClick={() =>
                      document.getElementById("sa-banner-input-new")?.click()
                    }
            ...
            <label className="border-2 border-dashed border-gray-300 rounded-xl p-8 flex flex-col items-center justify-center text-center hover:bg-gray-50 hover:border-blue-400 transition-colors cursor-pointer group relative">
              ...
              <input
                ...
                onChange={async (e) => {
                  ...
                  if (!file) return;
                  try {
                    const url = await uploadFile(file, "superadmin/admission");
                    setBrochureUrl(url);
                  } catch {
                    /* skip */
                  }
      ...
      {cropperOpen && cropImageSrc && (
        <ImageCropperModal
          ...
          onCropComplete={async (blob) => {
            ...
            try {
              const url = await uploadFile(croppedFile, "superadmin/admission");
              setHeroBanners((prev) => {
