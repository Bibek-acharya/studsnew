...
function getExpireStatus(expireDate: string | null): { label: string; style: string; date: string } {
  ...
  if (daysLeft < 0) return { label: "Expired", style: "text-red-600", date: dateStr };
  if (daysLeft <= 30) return { label: "Expires soon", style: "text-orange-600", date: dateStr };
...
export default function CollegeListSection({ setActiveSection }: { setActiveSection: (s: string) => void }) {
  ...
  const [paymentModal, setPaymentModal] = useState<{ open: boolean; institution: Institution | null }>({ open: false, institution: null });
  ...
  const fetchInstitutions = useCallback(async (filters?: { search?: string; type?: string; paymentStatus?: string; verification?: string; claim?: string; province?: string; level?: string }) => {
    setLoading(true);
    setAuthError(false);
    try {
      ...
      const response = await superadminFetch<ApiResponse>(`/api/v1/superadmin/institutions${qs ? `?${qs}` : ""}`);
  ...
  const refreshWithCurrentFilters = useCallback(() => {
    fetchInstitutions({ search: searchQuery, type: typeFilter, paymentStatus: paymentFilter, verification: verificationFilter, claim: claimFilter, province: provinceFilter, level: activeTab === "all" ? undefined : activeTab });
  }, [searchQuery, typeFilter, paymentFilter, verificationFilter, claimFilter, provinceFilter, activeTab, fetchInstitutions]);

  useEffect(() => { refreshWithCurrentFilters(); }, [refreshWithCurrentFilters]);

  const totalInstitutions = institutions.length;

  useEffect(() => { setCurrentPage(1); }, [searchQuery, typeFilter, paymentFilter, verificationFilter, claimFilter, provinceFilter]);

  useEffect(() => { setSelectAll(false); setSelectedIds(new Set()); }, [currentPage, perPage]);

  const filtered = institutions;
  const totalPages = Math.ceil(filtered.length / perPage);
  const paginated = filtered.slice((currentPage - 1) * perPage, currentPage * perPage);
  const startRecord = filtered.length === 0 ? 0 : (currentPage - 1) * perPage + 1;
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      await superadminFetch(`/api/v1/superadmin/institutions/${inst.id}/payment`, {
        method: "PUT",
        body: JSON.stringify({ payment_date: paymentDate, paid_for_days: paidForDays, amount: paymentAmount, remarks: paymentRemarks }),
      });
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (confirmDialog.action === "verify") {
        await superadminFetch(`/api/v1/superadmin/institutions/${id}/verify`, { method: "PUT" });
        showActionMsg("success", `${confirmDialog.institutionName} verification status updated`);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage and track all registered colleges</p>
        </div>
      ...
      {actionMsg && (
        <div className={`mb-4 flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium ${
          actionMsg.type === "success" ? "bg-green-50 border border-green-200 text-green-800" : "bg-red-50 border border-red-200 text-red-800"
        }`}>
          {actionMsg.type === "success" ? <CheckCircle className="w-5 h-5 text-green-600" /> : <CircleAlert className="w-5 h-5 text-red-600" />}
          {actionMsg.text}
        </div>
      )}
        ...
        <div className="p-6">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <div className="flex-1 min-w-[200px]">
              <div className="relative">
                ...
                <input type="text" placeholder="Search by college name, reg. number..." value={searchQuery}
                  onChange={e => setSearchQuery(e.target.value)}
                  className={`${inputClass} pl-10`} style={{ paddingRight: "0.875rem" }} />
              </div>
            </div>
            <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className={`${inputClass}`} style={{ width: "auto", minWidth: "150px" }}>
              {ORGANIZATION_TYPES.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
            <select value={paymentFilter} onChange={e => setPaymentFilter(e.target.value)} className={`${inputClass}`} style={{ width: "auto", minWidth: "150px" }}>
              {PAYMENT_STATUSES.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
            </select>
            <select value={verificationFilter} onChange={e => setVerificationFilter(e.target.value)} className={`${inputClass}`} style={{ width: "auto", minWidth: "150px" }}>
              {VERIFICATION_STATUSES.map(v => <option key={v.value} value={v.value}>{v.label}</option>)}
            </select>
            <select value={claimFilter} onChange={e => setClaimFilter(e.target.value)} className={`${inputClass}`} style={{ width: "auto", minWidth: "150px" }}>
              {CLAIM_STATUSES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
            </select>
            <select value={provinceFilter} onChange={e => setProvinceFilter(e.target.value)} className={`${inputClass}`} style={{ width: "auto", minWidth: "150px" }}>
          ...
          {selectedIds.size > 0 && (
            <div className="flex items-center justify-between mb-4 p-3 bg-blue-50 rounded-lg">
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-700"><span className="font-semibold">{selectedIds.size}</span> colleges selected</span>
              </div>
              <div className="flex items-center gap-2">
                <button type="button"
          ...
          {filtered.length === 0 ? (
            <div className="py-12 text-center text-gray-400">
              ...
              <p className="text-sm mt-1">Try adjusting your filters or add a new college.</p>
            </div>
          ) : (
            <>
                ...
                <table className="w-full text-sm min-w-[2400px]">
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      ...
                      <th className="text-left py-3 px-3 font-semibold text-gray-700 text-xs w-14">S.N</th>
                      ...
                      <th className="text-left py-3 px-3 font-semibold text-gray-700 text-xs w-44">Email</th>
                      <th className="text-left py-3 px-3 font-semibold text-gray-700 text-xs w-36">Phone</th>
                      <th className="text-center py-3 px-3 font-semibold text-gray-700 text-xs w-32">Account Type</th>
                      <th className="text-center py-3 px-3 font-semibold text-gray-700 text-xs w-28">Claim Status</th>
                      <th className="text-center py-3 px-3 font-semibold text-gray-700 text-xs w-28">Verified</th>
                      <th className="text-left py-3 px-3 font-semibold text-gray-700 text-xs w-36">Verified By</th>
                      <th className="text-center py-3 px-3 font-semibold text-gray-700 text-xs w-32">Payment</th>
                      <th className="text-left py-3 px-3 font-semibold text-gray-700 text-xs w-40">Last Payment</th>
                      <th className="text-center py-3 px-3 font-semibold text-gray-700 text-xs w-40">Expire Date</th>
                      <th className="text-left py-3 px-3 font-semibold text-gray-700 text-xs w-52">Remarks</th>
                      <th className="text-center py-3 px-3 font-semibold text-gray-700 text-xs w-44">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {paginated.map((inst, idx) => {
                      const sn = (currentPage - 1) * perPage + idx + 1;
                      const expireInfo = getExpireStatus(inst.subscription?.expire_date || null);
                      return (
                        <tr key={inst.id} className="hover:bg-gray-50 transition-colors">
                          <td className="py-3 px-3">
                            <input type="checkbox" checked={selectedIds.has(inst.id)} onChange={() => handleSelectOne(inst.id)}
                              className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
                          </td>
                          <td className="py-3 px-3 text-gray-600 text-xs">{sn}</td>
                          <td className="py-3 px-3 text-gray-600 text-xs">
                            <div>{formatDate(inst.created_at)}</div>
                            <div className="text-[10px] text-gray-400">{formatTime(inst.created_at)}</div>
                          </td>
                          <td className="py-3 px-3">
                            <div className="flex items-center gap-2">
                              {inst.logo_url ? (
                                <img src={inst.logo_url} className="w-8 h-8 rounded-lg object-contain flex-shrink-0" alt="" />
                              ) : (
                                <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${getInitialsColors(inst.institution_name)} flex items-center justify-center text-white text-xs font-semibold flex-shrink-0`}>
                                  {getInitials(inst.institution_name)}
                                </div>
                              )}
                              <div>
                                <p className="font-medium text-gray-900 text-xs">{inst.institution_name}</p>
                                <p className="text-[10px] text-gray-500">Reg: {inst.registration_number}</p>
                              </div>
                            </div>
                          </td>
                          <td className="text-center py-3 px-3">
                            <span className={`px-2 py-1 rounded text-xs font-semibold ${TYPE_STYLES[inst.organization_type] || "bg-gray-100 text-gray-700"}`}>
                          ...
                          <td className="py-3 px-3 text-gray-600 text-xs">
                            <div className="flex items-center gap-1">
                              ...
                              <div>
                                <p>{inst.district || "-"}</p>
                                <p className="text-[10px] text-gray-400">{inst.local_body ? `Ward-${inst.local_body}, ${inst.province}` : inst.province || ""}</p>
                              </div>
                            </div>
                          </td>
                          <td className="py-3 px-3">
                            <p className="text-gray-900 text-xs font-medium">{inst.contact_person || "-"}</p>
                            <p className="text-gray-500 text-[10px]">{inst.contact_person_designation || ""}</p>
                          </td>
                          <td className="py-3 px-3 text-gray-600 text-xs">{inst.email}</td>
                          <td className="py-3 px-3 text-gray-600 text-xs">{inst.contact_number || inst.contact_person_phone || "-"}</td>
                          <td className="text-center py-3 px-3">
                            {inst.subscription?.expire_date && new Date(inst.subscription.expire_date) > new Date() ? (
                              <span className="bg-indigo-100 text-indigo-700 px-2 py-1 rounded text-xs font-semibold">Premium</span>
                            ) : (
                              <span className="bg-gray-100 text-gray-600 px-2 py-1 rounded text-xs font-semibold">Free</span>
                            )}
                          </td>
                          <td className="text-center py-3 px-3">
                            {inst.claimed ? (
                              <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-semibold">Claimed</span>
                            ) : (
                              <span className="bg-amber-100 text-amber-700 px-2 py-1 rounded text-xs font-semibold">Unclaimed</span>
                            )}
                          </td>
                          <td className="text-center py-3 px-3">
                            <button type="button" onClick={() => handleOpenVerify(inst)}
                              ...
                              <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${inst.verified ? 'translate-x-[18px]' : 'translate-x-[2px]'}`} />
                            </button>
                          </td>
                          <td className="py-3 px-3">
                            {inst.verified_by ? (
                              <>
                                <p className="text-gray-900 text-xs">{inst.verified_by}</p>
                                {inst.verified_at && <p className="text-gray-500 text-[10px]">{formatDate(inst.verified_at)}</p>}
                              </>
                            ) : (
                          ...
                          <td className="text-center py-3 px-3">
                            <span className={`px-2 py-1 rounded text-xs font-semibold ${PAYMENT_STYLES[inst.subscription?.status || ""] || "bg-gray-100 text-gray-700"}`}>
                              {inst.subscription?.status ? inst.subscription.status.replace("_", " ").replace(/\b\w/g, l => l.toUpperCase()) : "N/A"}
                            </span>
                          </td>
                          <td className="py-3 px-3 text-gray-600 text-xs">
                            {inst.subscription?.last_payment_date ? (
                              <>
                                <div>{formatDate(inst.subscription.last_payment_date)}</div>
                                <div className="text-[10px] text-gray-400">Rs {inst.subscription.last_payment_amount.toLocaleString()}</div>
                              </>
                            ) : (
                          ...
                          <td className="py-3 px-3 text-gray-600 text-xs">
                            <div className={`text-xs font-medium ${expireInfo.style}`}>{expireInfo.date}</div>
                            <div className={`text-[10px] ${expireInfo.style}`}>{expireInfo.label}</div>
                          </td>
                          <td className="py-3 px-3 text-gray-600 text-xs">
                            {inst.subscription?.remarks ? (
                              <div className="flex items-center gap-1">
                                {inst.subscription.remarks.includes("verified") ? <CheckCircle className="w-3 h-3 text-green-500" /> :
                                 inst.subscription.remarks.includes("awaiting") ? <Clock className="w-3 h-3 text-yellow-500" /> :
                                 inst.subscription.remarks.includes("call") ? <Phone className="w-3 h-3 text-orange-500" /> :
                                 inst.subscription.remarks.includes("reminder") ? <CircleAlert className="w-3 h-3 text-red-500" /> :
                                 <Clock className="w-3 h-3 text-gray-400" />}
                                <span className="text-xs">{inst.subscription.remarks}</span>
                              </div>
                            ) : (
                              <span className="text-gray-400 text-xs">-</span>
                            )}
                          ...
                          <td className="text-center py-3 px-3">
                            <div className="flex items-center justify-center gap-1">
                              <button type="button" className="tooltip p-1.5 hover:bg-green-50 rounded text-green-600 transition-colors" title="Message"><MessageSquare className="w-3.5 h-3.5" /></button>
              ...
              <div className="flex flex-wrap items-center justify-between mt-6 pt-4 border-t border-gray-200 gap-4">
                <div className="flex items-center gap-4">
                  <p className="text-sm text-gray-500">
                    Showing <span className="font-semibold text-gray-700">{startRecord}-{endRecord}</span> of <span className="font-semibold text-gray-700">{filtered.length}</span> colleges
                  </p>
                  <select value={perPage} onChange={e => { setPerPage(Number(e.target.value)); setCurrentPage(1); }}
                    className={`${inputClass}`} style={{ width: "auto", padding: "0.5rem 2rem 0.5rem 0.75rem" }}>
                    ...
                    return (
                      <button key={pageNum} onClick={() => setCurrentPage(pageNum)}
                        className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${
                          currentPage === pageNum ? "bg-blue-600 text-white" : "border border-gray-200 text-gray-600 hover:bg-gray-50"
                        }`}>
                        {pageNum}
                      </button>
                    );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 mx-4">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                  <DollarSign className="w-5 h-5 text-yellow-600" /> Record Payment
                </h3>
                <p className="text-sm text-gray-500 mt-1">{paymentModal.institution.institution_name}</p>
              </div>
              <button type="button" onClick={() => setPaymentModal({ open: false, institution: null })}
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Paid For (Days)</label>
                <input type="number" min={1} value={paidForDays} onChange={e => setPaidForDays(Number(e.target.value))}
                  className={`${inputClass}`} placeholder="e.g. 365" />
                <p className="text-xs text-gray-400 mt-1">Expiry date will be: {paymentDate && paidForDays > 0 ? (() => {
              ...
              <div>
                ...
                <textarea value={paymentRemarks} onChange={e => setPaymentRemarks(e.target.value)}
                  className={`${inputClass} h-20 resize-none`} placeholder="Optional remarks about this payment..." />
              </div>
            </div>
            <div className="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-gray-200">
              ...
              <button type="button" onClick={handleSubmitPayment} disabled={paymentProcessing || !paymentDate || paidForDays <= 0}
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button type="button" onClick={handleConfirmAction}
                className={`px-4 py-2 rounded-lg text-sm font-medium text-white transition-colors flex items-center gap-2 ${
                  confirmDialog.action === "verify" ? "bg-green-600 hover:bg-green-700" :
                  confirmDialog.action === "suspend" ? "bg-orange-600 hover:bg-orange-700" : "bg-red-600 hover:bg-red-700"
                }`}>
                {confirmDialog.action === "verify" ? <CheckCircle className="w-4 h-4" /> :
                 confirmDialog.action === "suspend" ? <ShieldOff className="w-4 h-4" /> : <Trash2 className="w-4 h-4" />}
                {confirmDialog.action === "verify" ? "Verify" :
                 confirmDialog.action === "suspend" ? "Suspend" : "Delete"}
              </button>
            </div>
          </div>
        </div>
