...
import { Home, Newspaper, Search, Pencil, Trash2, ChevronLeft, ChevronRight, Plus } from "lucide-react";
...
export default function UniversityNewsSection({ setActiveSection }: { setActiveSection: (s: string) => void }) {
  ...
  const fetchNews = useCallback(async (p: number, s: string, uid: number) => {
    setLoading(true);
    try {
      const params: any = { page: p, limit, search: s || undefined };
      if (uid) params.university_id = uid;
      const res = await adminNewsApi.list(params);
      setNews(res.news || []);
  ...
  useEffect(() => { fetchNews(page, search, filterUniversity); }, [page, filterUniversity]);

  const handleSearch = () => { setPage(1); fetchNews(1, search, filterUniversity); };

  const confirmDelete = async () => {
    if (!deleteModal.id) return;
    const id = deleteModal.id;
    setDeleteModal({ isOpen: false, id: null, title: "" });
    try { await adminNewsApi.delete(id); setNews((prev) => prev.filter((n) => n.id !== id)); toast.success("News deleted."); }
      ...
      <div className="bg-white rounded-lg p-8 border border-slate-100">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setActiveSection("create-university-news")} className="flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-700"><Plus size={16} /> Add News</button>
          </div>
        </div>
                ...
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="text-center py-3 px-4">
                    <div className="flex items-center justify-center gap-2">
                      <button className="p-1.5 hover:bg-green-50 rounded text-green-600" onClick={() => setActiveSection(`edit-university-news-${item.id}`)}><Pencil className="w-4 h-4" /></button>
                      <button className="p-1.5 hover:bg-red-50 rounded text-red-600" onClick={() => { const t = news.find((n) => n.id === item.id); setDeleteModal({ isOpen: true, id: item.id, title: t?.title || "" }); }}><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              )})}
