  ...
  const confirmFeatured = useCallback(async () => {
    ...
    try {
      ...
      setNews((prev) =>
        prev.map((n) =>
          n.id === newsId
            ? { ...n, featured: setFeatured }
            : { ...n, featured: setFeatured ? false : n.featured },
        ),
      );
      toast.success(
        setFeatured
          ? "Featured story of the week set."
                ...
                news.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="text-center py-3 px-4">
                      <span
                        className={`px-2 py-1 rounded text-xs font-semibold ${CATEGORY_COLORS[item.category] || "bg-gray-100 text-gray-700"}`}
                      >
                    ...
                    <td className="text-center py-3 px-4">
                      <span
                        className={`px-2 py-1 rounded text-xs font-semibold ${item.published ? STATUS_BADGE.published : STATUS_BADGE.draft}`}
                      >
                    ...
                    <td className="text-center py-3 px-4">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        >
                          <Star
                            className={`w-4 h-4 ${item.featured ? "fill-yellow-500" : ""}`}
                          />
                        </button>
                        <button
                          ...
                          title="Edit"
                          onClick={() =>
                            setActiveSection(`edit-news-${item.id}`)
                          }
                        >
