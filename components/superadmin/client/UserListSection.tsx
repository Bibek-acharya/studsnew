...
export default function UserListSection() {
  ...
  const fetchUsers = (p: number, s: string) => {
    setLoading(true);
    apiService.listAllUsers({ search: s || undefined, page: p, limit: 20 })
      .then((res) => {
  ...
  useEffect(() => { fetchUsers(page, search); }, [page, search]); // eslint-disable-line
    ...
    <div className="rounded-xl border border-gray-100 bg-white p-8">
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-gray-100 bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-4 text-center font-bold text-gray-600">Last Login</th>
                ...
                <tr key={u.id} className="transition-colors hover:bg-gray-50">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      {u.image_url ? (
                        <img src={getImageUrl(u.image_url)} alt="" className="h-10 w-10 rounded-full object-cover" />
                      ) : (
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wider ${
                      u.status === "suspended" ? "bg-red-50 text-red-600" : "bg-green-50 text-green-600"
                    }`}>
                      {u.status || "Active"}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center text-xs text-gray-400">{formatDate(u.created_at)}</td>
                  <td className="px-6 py-4 text-center text-xs text-gray-400">{formatDateTime(u.last_login_at)}</td>
                  <td className="px-6 py-4">
      ...
      {totalPages > 1 && (
        <div className="mt-6 flex items-center justify-center gap-2">
          <button disabled={page <= 1} onClick={() => setPage(p => p - 1)} className="rounded-md border px-3 py-1.5 text-sm disabled:opacity-30">Prev</button>
          <span className="text-sm text-gray-500">Page {page} of {totalPages}</span>
          <button disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} className="rounded-md border px-3 py-1.5 text-sm disabled:opacity-30">Next</button>
        </div>
      )}

      {/* View User Detail Modal */}
      {viewUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => { setViewUser(null); setViewEducation([]); }}>
          <div className="mx-4 w-full max-w-lg rounded-2xl bg-white shadow-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 sticky top-0 bg-white rounded-t-2xl">
              <h3 className="text-lg font-bold text-gray-900">User Details</h3>
              <button onClick={() => setViewUser(null)} className="p-1 rounded-lg hover:bg-gray-100"><X size={18} /></button>
            </div>
            <div className="px-6 py-4 space-y-4">
              <div className="flex items-center gap-4">
                {viewUser.image_url ? (
                  <img src={getImageUrl(viewUser.image_url)} alt="" className="h-16 w-16 rounded-full object-cover border-2 border-gray-100" />
                ) : (
                  <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-br from-blue-400 to-blue-600 text-lg font-bold text-white">
                    {(viewUser.first_name?.[0] || "") + (viewUser.last_name?.[0] || "") || "U"}
                  </div>
                )}
                <div>
                  <h4 className="text-xl font-bold text-gray-900">{viewUser.first_name} {viewUser.last_name}</h4>
                  <p className="text-sm text-gray-500">{viewUser.email}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-4">
                <div><span className="text-xs text-gray-400">Phone</span><p className="text-sm font-medium">{viewUser.phone || "-"}</p></div>
                <div><span className="text-xs text-gray-400">Gender</span><p className="text-sm font-medium">{viewUser.gender || "-"}</p></div>
                <div><span className="text-xs text-gray-400">DOB</span><p className="text-sm font-medium">{viewUser.date_of_birth || "-"}</p></div>
                <div><span className="text-xs text-gray-400">Nationality</span><p className="text-sm font-medium">{viewUser.nationality || "-"}</p></div>
                <div><span className="text-xs text-gray-400">Status</span><p className="text-sm font-medium capitalize">{viewUser.status || "Active"}</p></div>
                <div><span className="text-xs text-gray-400">Joined</span><p className="text-sm font-medium">{formatDate(viewUser.created_at)}</p></div>
              </div>

              <div className="border-t border-gray-100 pt-4">
                <span className="text-xs text-gray-400">Last Login</span>
                <p className="text-sm font-medium">{formatDateTime(viewUser.last_login_at)}</p>
              </div>

              <div className="border-t border-gray-100 pt-4">
                <span className="text-xs text-gray-400">Address</span>
                <p className="text-sm font-medium">{parseAddr(viewUser.address)}</p>
              </div>
              <div className="border-t border-gray-100 pt-4">
              ...
              {viewUser.preferences && (
                <div className="border-t border-gray-100 pt-4">
                  <span className="text-xs text-gray-400 font-bold mb-3 block">Onboarding Data</span>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-xs text-gray-500">Status</span>
                      <span className={`text-sm font-medium ${viewUser.preferences.onboarding_completed ? "text-green-600" : "text-amber-600"}`}>
                        {viewUser.preferences.onboarding_completed ? "Completed" : "Incomplete"}
                      </span>
                    </div>
                    {viewUser.preferences.completed_at && (
                      <div className="flex justify-between">
                        <span className="text-xs text-gray-500">Completed At</span>
                        <span className="text-sm font-medium text-gray-800">{formatDateTime(viewUser.preferences.completed_at)}</span>
                      </div>
                    )}
              ...
              {viewEducation.length > 0 && (
                <div className="border-t border-gray-100 pt-4">
                  <span className="text-xs text-gray-400 font-bold mb-3 block">Academic History</span>
                  <table className="w-full text-xs border border-gray-100 rounded-md overflow-hidden">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-3 py-2 text-left font-bold text-gray-500">Level</th>
                        <th className="px-3 py-2 text-left font-bold text-gray-500">Institution</th>
                        <th className="px-3 py-2 text-left font-bold text-gray-500">Board/Univ</th>
                        <th className="px-3 py-2 text-left font-bold text-gray-500">Stream</th>
                        <th className="px-3 py-2 text-left font-bold text-gray-500">Grade</th>
                        <th className="px-3 py-2 text-left font-bold text-gray-500">Year</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {viewEducation.map((e: any) => (
                        <tr key={e.id}>
                          ...
                          <td className="px-3 py-2 text-gray-500">{e.institution_name}</td>
                          <td className="px-3 py-2 text-gray-500">{e.board_university}</td>
                          <td className="px-3 py-2 text-gray-500">{e.stream || "-"}</td>
                          <td className="px-3 py-2">{e.grade || "-"} {e.grading_system ? `(${e.grading_system})` : ""}</td>
                          <td className="px-3 py-2 text-gray-400 whitespace-nowrap">{e.start_year} - {e.end_year}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
