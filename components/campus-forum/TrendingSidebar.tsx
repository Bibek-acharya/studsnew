      ...
      <div className="bg-white rounded-[1.5rem] shadow-sm border border-slate-100 overflow-hidden group hover:shadow-lg transition-all duration-500 pb-4">
        ...
        <div className="space-y-0 px-2">
          {[
            ...
            <div
              key={item.id}
              className="p-4 transition-colors border-b border-slate-50 last:border-0 hover:bg-slate-50 cursor-pointer rounded-xl mx-2"
            >
        ...
        <div className="px-6 mt-4">
          <button className="w-full py-3 bg-slate-50 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all">
            See all notices
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[1.5rem] shadow-sm border border-slate-100 p-6">
        ...
        <div className="flex flex-wrap gap-2">
          {["#CSIT", "#Scholarship2026", "#CEE_Preparation", "#TU_Notices", "#Engineering", "#BCA", "#LokSewa"].map((tag) => (
            <button key={tag} className="px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl text-xs font-bold text-slate-600 hover:bg-white hover:border-blue-200 hover:text-blue-600 transition-all">
              {tag}
            </button>
          ))}
        </div>
