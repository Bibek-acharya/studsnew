...
const LoginView: React.FC<LoginViewProps> = ({
  ...
  const { values, touched, errors, setValue, touch, validateField, setErrors } = useFieldValidation({
    email: "",
    password: "",
  });
  ...
  const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (touched.password) validateField("password", val, validators.password);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const passwordError = validateField("password", values.password, validators.password);
    if (emailError || passwordError) return;

    setLoading(true);
    try {
      await login(values.email, values.password);
      onSuccess({ name: values.email.split("@")[0], email: values.email, role: "student" });
    } catch (err: any) {
  ...
  return (
    <div className="w-full max-w-[360px] mx-auto flex flex-col py-4">
      ...
      <h1 className="text-xl font-bold text-gray-900 mb-1">Log in to your Account</h1>
      <p className="text-gray-500 text-sm mb-5">Welcome back! Select method to log in:</p>
        ...
        <div>
          <div className="relative">
            ...
            <input
              ...
              required
              disabled={loading}
              className={`w-full pl-11 pr-4 py-2.5 border rounded-lg text-sm focus:ring-1 outline-none transition-colors placeholder:text-gray-400 disabled:bg-gray-50 ${getBorderClass("email")} ${touched.email && !errors.email ? "focus:border-green-400 focus:ring-green-400" : errors.email && touched.email ? "focus:border-red-400 focus:ring-red-400" : "focus:border-blue-600 focus:ring-blue-600"}`}
              value={values.email}
        ...
        <div>
          <div className="relative">
            ...
            <input
              ...
              required
              disabled={loading}
              className={`w-full pl-11 pr-11 py-2.5 border rounded-lg text-sm focus:ring-1 outline-none transition-colors placeholder:text-gray-400 disabled:bg-gray-50 ${getBorderClass("password")} ${touched.password && !errors.password ? "focus:border-green-400 focus:ring-green-400" : errors.password && touched.password ? "focus:border-red-400 focus:ring-red-400" : "focus:border-blue-600 focus:ring-blue-600"}`}
