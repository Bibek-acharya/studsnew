...
const UniversityDetail: React.FC = () => {
  ...
  const overviewList = uni?.overview ? (Array.isArray(uni.overview) ? uni.overview : []) : [];
  const leadershipList = uni?.leadership ? (Array.isArray(uni.leadership) ? uni.leadership : []) : [];
  const coursesList = uni?.courses ? (Array.isArray(uni.courses) ? uni.courses : []) : [];
  const admissionsList = uni?.admissions ? (Array.isArray(uni.admissions) ? uni.admissions : []) : [];
  const scholarshipsList = uni?.scholarships ? (Array.isArray(uni.scholarships) ? uni.scholarships : []) : [];
  const eventsList = uni?.events ? (Array.isArray(uni.events) ? uni.events : []) : [];
  const newsList = uni?.news ? (Array.isArray(uni.news) ? uni.news : []) : [];
  const downloadsList = uni?.downloads ? (Array.isArray(uni.downloads) ? uni.downloads : []) : [];
  const galleryList = uni?.gallery ? (Array.isArray(uni.gallery) ? uni.gallery : []) : [];
  const institutesList = uni?.faculties ? (Array.isArray(uni.faculties) ? uni.faculties : []) : [];
  ...
  if (error || !uni) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <p className="text-gray-500 text-lg font-medium mb-4">Failed to load university details.</p>
        <button
  ...
  return (
    ...
    <div className="w-full bg-white font-sans">
      ...
      <div
        ...
        style={uni?.cover ? { backgroundImage: `url(${uni.cover})` } : undefined}
      />
          ...
          <div className="flex flex-col items-start justify-between pt-20 lg:flex-row lg:items-end lg:pt-6 lg:ml-[180px]">
            <div className="w-full space-y-3 lg:w-auto">
              <div className="flex items-center gap-2">
                ...
                {uni?.verified && <BadgeCheck className="h-6 w-6 fill-blue-500 text-white" />}
              </div>
              <div className="flex items-center gap-1.5 pt-1">
                <Star className="h-4 w-4 fill-blue-500 text-blue-500" />
                <span className="text-[14px] font-bold text-gray-900">{uni?.rating ?? "—"}</span>
                <span className="text-[14px] text-gray-500">({uni?.review_count ?? 0} Reviews)</span>
              </div>
              <button
                type="button"
                onClick={() => {
          ...
          <div className="lg:col-span-2">
            ...
            {activeTab === "tab-about" && (
              <div className="space-y-10">
                ...
                {(aboutData?.vision || aboutData?.mission || aboutData?.values) && (
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                    {aboutData?.vision && (
                      <div className="rounded-[20px] bg-[#f4f7fb] p-8">
                        ...
                        <p className="text-[14.5px] leading-[1.7] text-gray-600">{aboutData.vision as string}</p>
                      </div>
                    )}
                    {aboutData?.mission && (
                      <div className="rounded-[20px] bg-[#f0fdf4] p-8">
                        ...
                        <p className="text-[14.5px] leading-[1.7] text-gray-600">{aboutData.mission as string}</p>
                      </div>
                    )}
                    {aboutData?.values && (
                      <div className="rounded-[20px] bg-[#fef2f2] p-8">
                        ...
                        <p className="text-[14.5px] leading-[1.7] text-gray-600">{aboutData.values as string}</p>
                      </div>
                    )}
                  </div>
                )}
                  ...
                  <div className="overflow-hidden rounded-[20px] border border-gray-100 bg-white shadow-sm">
                    ...
                    <div className="divide-y divide-gray-100">
                      {overviewList.map((row: any, idx: number) => (
                        <div key={idx} className="flex flex-col p-4 transition-colors hover:bg-gray-50 sm:flex-row">
                          <div className="w-full text-[14px] font-semibold text-gray-800 sm:w-1/3">{row.label || row.key || row.field}</div>
                          <div className="w-full text-[14px] text-gray-600 sm:w-2/3">{row.value || row.val || String(row)}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                      ...
                      <table className="w-full text-left text-[14px] text-gray-600">
                        ...
                        <tbody className="divide-y divide-gray-100">
                          {leadershipList.map((row: any, idx: number) => (
                            <tr key={idx}>
                              <td className="px-6 py-4 font-bold text-gray-900">{row.position || row.title}</td>
                              <td className="px-6 py-4">{row.role || ""}</td>
                              <td className="px-6 py-4 font-semibold">{row.holder || row.name || row.person}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                {aboutData?.highlights && Array.isArray(aboutData.highlights) && aboutData.highlights.length > 0 && (
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    {aboutData.highlights.map((h: any, idx: number) => (
                      <div key={idx} className="flex items-start gap-4 rounded-xl border border-blue-100 bg-blue-50 p-5">
                        ...
                        <div><h4 className="text-[15px] font-bold text-gray-900">{h.title || h.label}</h4><p className="mt-1 text-[14px] text-gray-600">{h.description || h.text || h.desc}</p></div>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="w-full overflow-x-auto">
                  <div className="min-w-[700px]">
                    ...
                    {coursesList.length > 0 ? (
                      coursesList
                        .filter((c: any) => courseFilter === "all" || c.level === courseFilter)
                        .map((course: any, i: number) => (
                          <div key={i} className="grid grid-cols-12 gap-4 border-b border-gray-100 px-6 py-5 transition-colors hover:bg-gray-50/50 items-center">
                            <div className="col-span-4 pr-4"><h4 className="text-[15.5px] font-bold text-gray-900">{course.name}</h4><p className="mt-1 text-[12px] text-gray-500">{course.sub || course.description || ""}</p></div>
                  ...
                  <div className="grid grid-cols-1 gap-5">
                      ...
                      <div className="rounded-[16px] border border-gray-100 bg-white p-5 shadow-sm">
                        <div className="flex cursor-pointer items-center justify-between" onClick={() => toggleDropdown("affiliated")}>
                          <div className="flex items-center gap-3">
                            ...
                            <h4 className="text-[17px] font-bold text-gray-900">Affiliated Colleges ({colleges.length})</h4>
                          </div>
                        ...
                        {openDropdowns["affiliated"] && (
                          <div className="mt-6">
                            <table className="prog-table w-full">
                              <thead><tr><th className="bg-[#f8fafc] px-2 py-2.5 text-left text-[13px] font-semibold text-[#1e293b]">#</th><th className="bg-[#f8fafc] px-2 py-2.5 text-left text-[13px] font-semibold text-[#1e293b]">College</th><th className="bg-[#f8fafc] px-2 py-2.5 text-left text-[13px] font-semibold text-[#1e293b]">Type</th><th className="bg-[#f8fafc] px-2 py-2.5 text-left text-[13px] font-semibold text-[#1e293b]">Rating</th></tr></thead>
                              <tbody>
                                {colleges.map((col, idx) => (
                                  <tr key={col.id} className="border-b border-gray-50">
                                    <td className="px-2 py-2 text-[13px] text-[#334155]">{idx + 1}</td>
                                    <td className="px-2 py-2 text-[13px] text-[#334155]">{col.name}</td>
                                    <td className="px-2 py-2 text-[13px] text-[#334155]">{col.affiliation || col.type}</td>
                                    <td className="px-2 py-2 text-[13px] text-[#334155]">{col.rating ?? "—"}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
                    ...
                    {institutesList.length > 0 ? (
                      institutesList.map((inst: any, idx: number) => (
                        <div key={idx} className="rounded-[16px] border border-gray-100 bg-white p-5 shadow-sm">
                          <div className="flex cursor-pointer items-center justify-between" onClick={() => toggleDropdown(`inst-${idx}`)}>
                            <div className="flex items-center gap-3">
                              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-50"><Building2 className="h-5 w-5 text-blue-600" /></div>
                              <h4 className="text-[15px] font-bold text-gray-900">{inst.title || inst.name || `Institute ${idx + 1}`}</h4>
                            </div>
                            <ChevronDown className={`h-4 w-4 text-gray-500 transition-transform ${openDropdowns[`inst-${idx}`] ? "rotate-180" : ""}`} />
                          </div>
                          {openDropdowns[`inst-${idx}`] && (
                            <div className="mt-6">
                              <table className="prog-table w-full">
                                <thead><tr><th className="bg-[#f8fafc] px-2 py-2.5 text-left text-[13px] font-semibold text-[#1e293b]">#</th><th className="bg-[#f8fafc] px-2 py-2.5 text-left text-[13px] font-semibold text-[#1e293b]">Detail</th></tr></thead>
                                <tbody>
                                  {(inst.items || inst.colleges || inst.programs || []).map((item: any, ii: number) => (
                                    <tr key={ii} className="border-b border-gray-50">
                                      <td className="px-2 py-2 text-[13px] text-[#334155]">{ii + 1}</td>
                                      <td className="px-2 py-2 text-[13px] text-[#334155]">{item.name || item.title || item}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
                  ...
                  <div className="grid grid-cols-1 gap-5">
                    {uni?.programs && Array.isArray(uni.programs) && uni.programs.length > 0 ? (
                      uni.programs.map((fac: any, idx: number) => (
                        <div key={idx} className="rounded-[16px] border border-gray-100 bg-white p-5 shadow-sm">
                          <div className="flex cursor-pointer items-center justify-between" onClick={() => toggleDropdown(`fac-${idx}`)}>
                            <div className="flex items-center gap-3">
                              <div className="flex h-10 w-10 items-center justify-center rounded-full bg-green-50"><BookOpen className="h-5 w-5 text-green-600" /></div>
                              <h4 className="text-[17px] font-bold text-gray-900">{fac.title || fac.name || `Faculty ${idx + 1}`}</h4>
                            </div>
                            <ChevronDown className={`h-5 w-5 text-gray-500 transition-transform ${openDropdowns[`fac-${idx}`] ? "rotate-180" : ""}`} />
                          </div>
                          {openDropdowns[`fac-${idx}`] && (
                            <div className="mt-6">
                              <table className="prog-table w-full">
                                <thead><tr><th className="bg-[#f8fafc] px-2 py-2.5 text-left text-[13px] font-semibold text-[#1e293b]">#</th><th className="bg-[#f8fafc] px-2 py-2.5 text-left text-[13px] font-semibold text-[#1e293b]">Programs</th><th className="bg-[#f8fafc] px-2 py-2.5 text-left text-[13px] font-semibold text-[#1e293b]">Duration</th></tr></thead>
                                <tbody>
                                  {(fac.programs || fac.items || []).map((p: any, pi: number) => (
                                    <tr key={pi} className="border-b border-gray-50">
                                      <td className="px-2 py-2 text-[13px] text-[#334155]">{pi + 1}</td>
                                      <td className="px-2 py-2 text-[13px] text-[#334155]">{p.name || p.title || p}</td>
                      ...
                      <div key={i} className="flex h-full w-full max-w-85 flex-col overflow-hidden rounded-md border border-gray-200 bg-white transition-transform hover:border-blue-200 cursor-pointer">
                        <div className="shrink-0 p-2.5 pb-0">
                          <div className="group relative h-28 w-full overflow-hidden rounded-md bg-gray-200">
                            <img src={ad.image || (i === 0 ? "https://images.unsplash.com/photo-1523240795612-9a054b0db644?q=80&w=400&auto=format&fit=crop" : "https://images.unsplash.com/photo-1515162816999-a0c47dc192f7?q=80&w=400&auto=format&fit=crop")} className="h-full w-full object-cover" alt="" />
                      ...
                      scholarshipsList
                        ...
                        .map((sch: any, i: number) => (
                          <div key={i} className="grid grid-cols-12 gap-4 border-b border-gray-100 px-6 py-5 transition-colors hover:bg-gray-50/50 items-center">
                            ...
                            <div className="col-span-2"><h4 className="text-[14px] font-bold text-gray-900">{sch.name || sch.title || ""}</h4></div>
                            <div className="col-span-2"><span className="text-[13px] font-medium text-green-600">{sch.benefit || ""}</span></div>
                            <div className="col-span-3"><span className="text-[13px] text-gray-600">{sch.forWhom || sch.eligibility || ""}</span></div>
                    ...
                    {eventsList.map((ev: any, i: number) => (
                      <div key={i} className="flex flex-col overflow-hidden rounded-md border border-gray-200 bg-white transition-colors hover:border-blue-500/20 duration-300">
                        <div className="h-35 w-full overflow-hidden p-4">
                          <img src={ev.image || "https://images.unsplash.com/photo-1540575467063-178a50c2df87?q=80&w=800&auto=format&fit=crop"} alt={ev.title} className="h-full w-full rounded-md object-cover" />
                        </div>
                        <div className="flex grow flex-col p-5">
                          <div className="mb-3 flex items-center justify-between">
                            <span className="rounded-full bg-teal-500 px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-white">{ev.month || ""}</span>
                            <span className="flex items-center text-xs font-semibold text-gray-500"><i className="fa-regular fa-calendar mr-1.5"></i> {ev.date || ""} {ev.month || ""}</span>
                          </div>
                          ...
                          <div className="mb-3 flex items-center text-xs font-semibold text-gray-600"><i className="fa-solid fa-location-dot mr-2 text-gray-500"></i>{ev.location || "Central Campus"}</div>
                          <p className="mb-5 line-clamp-3 text-xs font-medium leading-relaxed text-gray-500">{ev.description || ev.desc || ""}</p>
                    ...
                    {newsList.map((item: any, i: number) => (
                      <div key={i} className="flex flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-shadow hover:shadow-md">
                        <div className="flex flex-1 flex-col p-5">
                          <div className="mb-4"><span className="inline-block rounded-full bg-blue-100 px-3.5 py-1 text-[12px] font-bold text-blue-700">{item.tag || "News"}</span></div>
                          <div className="mb-4 h-[140px] w-full shrink-0 overflow-hidden rounded-xl">
                            <img src={item.image || item.img || ""} alt="" className="h-full w-full object-cover transition-transform duration-500 hover:scale-105" />
                          </div>
                          <h3 className="mb-2 text-[17px] font-bold leading-tight text-gray-900">{item.title}</h3>
                          <p className="mb-2 line-clamp-2 text-[13.5px] text-gray-500">{item.description || item.desc || ""}</p>
                        </div>
                        <div className="mt-auto flex items-center justify-between border-t border-gray-50 bg-white px-5 py-4">
                          <div className="flex items-center gap-1.5 text-gray-400"><Clock className="h-4 w-4" /><span className="text-[12.5px] font-medium">{item.time || item.date || ""}</span></div>
                  ...
                  {downloadsList.length > 0 ? (
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      {downloadsList.map((dl: any, i: number) => (
                        <div key={i} className="flex items-start gap-3 rounded-xl border border-gray-100 bg-[#f8fafc] p-5 transition-all hover:border-blue-200">
                          <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-blue-50">
                            <FileDown className="h-6 w-6 text-blue-600" />
                          </div>
                          <div className="flex-1">
                            <h4 className="text-[15px] font-bold text-gray-900">{dl.title || dl.name}</h4>
                            <p className="mb-2 text-[12px] text-gray-500">{dl.meta || dl.description || dl.size || ""}</p>
                            <a href={dl.url || dl.link || "#"} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-[13px] font-bold text-blue-600 transition-colors hover:text-blue-700">
              ...
              <div className="space-y-10">
                {galleryList.length > 0 ? (
                  (() => {
                    ...
                    const urls = galleryList.map((i: any) => i.url || i.image || i.src);
                    return Array.from(groups.entries()).map(([heading, items], gi) => (
                      <div key={gi} className="space-y-5">
                        ...
                        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4 md:gap-5">
                          {items.slice(0, items.length > 8 ? 7 : 8).map((img: any, ii: number) => {
                            ...
                            return (
                              ...
                              >
                                <div className="aspect-[4/3] overflow-hidden rounded-xl bg-gray-50">
                                  <img src={url} alt={img.title || img.alt || ""} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110" />
                                </div>
                              </div>
                            );
                          })}
                          {items.length > 8 && (
                            <div
                              ...
                              onClick={() => {
                                const firstUrl = (items[0] as any).url || (items[0] as any).image || (items[0] as any).src;
              ...
              <div>
                <div className="mb-8 flex flex-col items-center gap-8 rounded-md border border-gray-200 bg-white p-8 md:flex-row">
                  <div className="text-center md:border-r md:pr-8 md:text-left">
                    <h2 className="mb-2 text-5xl font-extrabold text-gray-900">{uni?.rating ?? "—"}</h2>
                    <div className="mb-2 flex items-center justify-center gap-1 md:justify-start">
                      {Array.from({ length: 5 }).map((_, idx) => (
                        <i key={idx} className={`fa-solid fa-star text-[14px] ${idx < Math.round(uni?.rating ?? 0) ? "text-yellow-400" : "text-gray-300"}`}></i>
                      ))}
                    </div>
                    <p className="text-[13px] font-medium text-gray-500">Based on {uni?.review_count ?? 0} reviews</p>
                  </div>
                ...
                <div className="space-y-5">
                  {uni?.reviews && Array.isArray(uni.reviews) && uni.reviews.length > 0 ? (
                    uni.reviews.map((review: any, idx: number) => (
                      <ReviewCard
                        key={idx}
                        initials={(review.name || "A").charAt(0).toUpperCase()}
                        name={review.name}
          ...
          <div className="space-y-6 lg:col-span-1 lg:w-full lg:max-w-[400px] lg:ml-8 xl:ml-12">
            <div className="w-full rounded-2xl border border-gray-200 bg-white p-4 sm:p-10">
              ...
              <div className="flex flex-col gap-6">
                ...
                />
                {(contactData?.social || contactData?.facebook || contactData?.twitter || contactData?.instagram || contactData?.youtube || contactData?.linkedin) && (
                  <div className="w-full">
                    ...
                    <div className="mt-3 flex gap-5 text-[26px]">
                      {(contactData?.facebook as string) && (
                        <a href={contactData.facebook as string} className="text-[#1877F2] transition-transform hover:scale-110" title="Facebook" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-facebook"></i></a>
                      )}
                      {(contactData?.instagram as string) && (
                        <a href={contactData.instagram as string} className="text-[#E4405F] transition-transform hover:scale-110" title="Instagram" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-instagram"></i></a>
                      )}
                      {(contactData?.tiktok as string) && (
                        <a href={contactData.tiktok as string} className="text-black transition-transform hover:scale-110" title="TikTok" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-tiktok"></i></a>
                      )}
                      {(contactData?.youtube as string) && (
                        <a href={contactData.youtube as string} className="text-[#FF0000] transition-transform hover:scale-110" title="YouTube" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-youtube"></i></a>
                      )}
                      {(contactData?.linkedin as string) && (
                        <a href={contactData.linkedin as string} className="text-[#0A66C2] transition-transform hover:scale-110" title="LinkedIn" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-linkedin"></i></a>
                      )}
                    </div>
                  </div>
                )}
                {(contactData?.mapUrl || contactData?.address) && (
                  <div className="mt-8 h-40 w-full overflow-hidden rounded-md">
                    <iframe
                      src={contactData?.mapUrl as string || `https://www.google.com/maps?q=${encodeURIComponent(contactData?.address as string || "")}&output=embed`}
                      width="100%"
                      height="100%"
                      style={{ border: 0 }}
                      allowFullScreen
      ...
      {lightboxIndex !== null && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/95" onClick={() => setLightboxIndex(null)}>
          ...
          <img src={(galleryList[lightboxIndex] as any)?.url || (galleryList[lightboxIndex] as any)?.image || (galleryList[lightboxIndex] as any)?.src || ""} alt={(galleryList[lightboxIndex] as any)?.title || (galleryList[lightboxIndex] as any)?.alt || "Gallery"} className="max-h-[85vh] max-w-[90%] rounded-md object-contain" onClick={(e) => e.stopPropagation()} />
