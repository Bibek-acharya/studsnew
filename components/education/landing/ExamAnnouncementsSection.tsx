...
import React, { useState, type MouseEvent, type SyntheticEvent } from "react";
...
const examTags = (exam: any): { text: string; icon: React.ReactNode; type: "alert" | "warning" | "success" | "purple" | "default" }[] => {
  const tags: { text: string; icon: React.ReactNode; type: "alert" | "warning" | "success" | "purple" | "default" }[] = [];
  if (exam.status === "open" || exam.status === "ongoing") tags.push({ text: "Applications Open", icon: <BadgeCheck className="w-3 h-3" />, type: "purple" });
  else if (exam.status === "upcoming") tags.push({ text: "Upcoming", icon: <Clock className="w-3 h-3" />, type: "default" });
  else tags.push({ text: "Closed", icon: <Clock className="w-3 h-3" />, type: "alert" });
  if (exam.type) tags.push({ text: exam.type, icon: <FileText className="w-3 h-3" />, type: "default" });
  return tags;
};

const ExamAnnouncementsSection: React.FC<ExamAnnouncementsSectionProps> = ({
  onNavigate: _onNavigate, exams = [],
}) => {
  void _onNavigate;
              ...
              <header className="flex justify-between items-start mb-4 sm:mb-5">
                <div className="flex gap-2.5 sm:gap-3">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-md border border-gray-200 flex items-center justify-center bg-white shrink-0">
                    <Image
                      src={exam.institution_logo || "https://placehold.co/48x48/f1f5f9/94a3b8?text=Logo"}
                      alt={exam.institution_name || "Institution"}
                      width={48}
                      height={48}
                      unoptimized
                  ...
                  <div className="flex flex-col min-w-0">
                    <h3 className="text-[13px] xs:text-[14px] sm:text-[15px] font-bold text-[#111827] hover:text-[#0000ff] flex items-center gap-1 sm:gap-1.5 truncate">
                      {exam.institution_name || exam.board || "Institution"}
                      {true && (
                ...
                <div className="bg-[#f8fafc] rounded-md sm:rounded-md p-2 sm:p-2.5 flex flex-col gap-1.5 sm:gap-2 mt-auto border border-[#f1f5f9]">
                  <div className="flex items-center gap-2 sm:gap-2.5 text-[11px] xs:text-[12px] sm:text-[13px] text-[#475569]">
                    ...
                    <span className="truncate font-medium text-red-500 text-[11px] sm:text-[12px]">
                      Deadline: {exam.form_deadline ? new Date(exam.form_deadline).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" }) : "TBA"}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 sm:gap-2.5 text-[11px] xs:text-[12px] sm:text-[13px] text-[#475569]">
                    <Calendar className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#94a3b8] shrink-0" />
                    <span className="truncate text-[11px] sm:text-[12px]">
                      Exam: {exam.exam_date ? new Date(exam.exam_date).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" }) : "TBA"}
                    </span>
                  </div>
                  {exam.highlights && exam.highlights.length > 0 && (
                  <div className="flex items-center gap-2 sm:gap-2.5 text-[11px] xs:text-[12px] sm:text-[13px] text-[#475569]">
                    <Award className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#94a3b8] shrink-0" />
                    <span className="truncate text-[11px] sm:text-[12px]">{exam.highlights.join(", ")}</span>
                  </div>
                  )}
                </div>
              </main>
