  ...
  const [invites, setInvites] = useState([
    {
      ...
      description:
        "Full tuition waiver for top 5% scorers in entrance examination.",
      amount: "100% Waiver",
      deadline: "2026-03-15",
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1 mt-16 md:mt-0">
          ...
          <div className="pt-2 border-t border-slate-100 mt-4">
            ...
            <button
              ...
              className="flex items-center px-4 py-3 text-sm font-medium rounded-lg text-red-500 hover:bg-red-50 w-full"
            >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {activeTab === "dashboard" && (
            <div className="space-y-8 animate-in fade-in duration-500">
              ...
              <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl p-4 text-white shadow-lg flex items-center justify-between">
                ...
                <button className="bg-white text-blue-700 px-4 py-2 rounded-lg text-xs font-bold hover:bg-blue-50">
                  Join Now
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl p-6 text-white shadow-lg relative overflow-hidden group hover:shadow-xl transition-all cursor-pointer">
                  <div className="absolute -right-6 -bottom-6 opacity-10 scale-150 transform rotate-12 transition-transform group-hover:scale-175">
                    <HelpCircle size={100} />
                  </div>
                  ...
                  <button className="mt-4 bg-white text-purple-600 px-4 py-2 rounded-lg text-xs font-bold uppercase relative z-10">
                    Write a Review
                  </button>
                </div>
                <div className="bg-gradient-to-r from-orange-500 to-red-500 rounded-xl p-6 text-white shadow-lg relative overflow-hidden group hover:shadow-xl transition-all cursor-pointer">
                  ...
                  <button className="mt-4 bg-white text-orange-600 px-4 py-2 rounded-lg text-xs font-bold uppercase relative z-10">
                    Ask Now
                  </button>
                </div>
              </div>
                ...
                {[
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <div
                        className={`p-2 rounded-lg flex items-center justify-center bg-${stat.color}-50 text-${stat.color}-600`}
                      >
                        <stat.icon size={24} />
                      </div>
                    </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    <button
                      key={i}
                      className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 hover:border-blue-500 transition-all text-center group"
                    >
                      <div
                        className={`w-10 h-10 mx-auto rounded-full flex items-center justify-center mb-2 bg-${act.color}-50 text-${act.color}-600 group-hover:bg-blue-600 group-hover:text-white transition-colors`}
                      >
                        <act.icon size={20} />
                      </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {Object.entries(EVENT_TYPES).map(([key, val]) => (
                        ...
                        >
                          <div
                            className={`w-5 h-5 rounded border border-slate-200 flex items-center justify-center transition-all ${activeFilters.includes(key) ? "bg-current border-transparent" : "bg-white"}`}
                          ...
                          />
                          <span
                            className={`text-sm font-medium ${activeFilters.includes(key) ? "text-slate-700" : "text-slate-400"}`}
                          >
                            {val.label}
                          </span>
                        </label>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {checklist.map((item) => (
                        ...
                        >
                          <div
                            className={`w-4 h-4 rounded border flex items-center justify-center ${item.checked ? "bg-emerald-500 border-emerald-500 text-white" : "border-slate-300"}`}
                          >
                ...
                <header className="p-6 border-b border-slate-100 flex items-center justify-between">
                  ...
                  <div className="hidden sm:flex bg-slate-100 p-1 rounded-lg">
                    {["month", "week", "day"].map((v) => (
                      <button
                        key={v}
                        className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${calendarView === v ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                ...
                <div className="flex-1 overflow-auto">
                  <div className="grid grid-cols-7 border-b border-slate-100 sticky top-0 bg-white z-10">
                    {DAYS.map((d) => (
                      <div
                        key={d}
                        className="py-2 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest"
                      >
                        {d}
                      </div>
                    ))}
                  ...
                  <div className="grid grid-cols-7 border-l border-slate-50 min-h-full">
                    ...
                    {Array.from({ length: 35 }).map((_, i) => (
                      <div
                        key={i}
                        className="border-r border-b border-slate-50 min-h-[100px] p-2 hover:bg-slate-50 transition-colors cursor-pointer group"
                      >
                        <span className="text-sm font-semibold text-slate-400 group-hover:text-blue-600">
                          {i - 2 > 0 && i - 2 <= 30 ? i - 2 : ""}
                        </span>
            ...
            <div className="space-y-6 animate-in fade-in duration-500">
              <div>
                <h2 className="text-2xl font-bold flex items-center gap-2">
                  <Mail className="text-blue-600" /> SphereInvites
                </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                  ...
                  <div
                    key={i}
                    className="bg-white p-5 rounded-xl shadow-sm border border-slate-100 flex items-center gap-4"
                  >
                    <div
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pt-4">
                {invites.map((inv) => (
                  <div
                    key={inv.id}
                    className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden flex flex-col group hover:shadow-lg transition-all relative"
                  >
                    {inv.priority && (
                    ...
                    <div className="p-5 flex-1">
                      <div className="flex items-center gap-3 mb-4">
                        <div className="h-10 w-10 bg-slate-50 border rounded-lg flex items-center justify-center font-bold text-slate-700">
                          {inv.logo}
                        </div>
                        <div>
                      ...
                      <span className="inline-block px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-blue-50 text-blue-700 mb-2">
                        {inv.type}
                      </span>
                      <h4 className="font-bold text-slate-800 leading-snug mb-2">
                        {inv.title}
                      </h4>
                      ...
                      {inv.amount && (
                        <div className="bg-slate-50 p-2 rounded-lg text-xs font-bold flex items-center gap-2 border border-slate-100">
                    ...
                    <div className="p-4 bg-slate-50/50 border-t border-slate-100 grid grid-cols-2 gap-3">
                      <button className="py-2 text-slate-600 bg-white border border-slate-200 rounded-lg text-xs font-bold hover:bg-slate-100">
                        Decline
                      </button>
                      <button className="py-2 bg-blue-600 text-white rounded-lg text-xs font-bold hover:bg-blue-700">
                        Accept
                      </button>
                    </div>
                  </div>
          ...
          {!["dashboard", "calendar", "sphereinvites"].includes(activeTab) && (
            <div className="flex flex-col items-center justify-center min-h-[400px] text-slate-400">
              <span className="p-4 rounded-full bg-slate-100 mb-4">
                <Menu size={32} />
              </span>
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-sm w-full relative z-[301] text-center animate-in zoom-in duration-300">
            ...
            <div className="flex gap-4">
              ...
              <button
                onClick={() => {
                  setIsLogoutModalOpen(false); /* Add logout logic */
                }}
