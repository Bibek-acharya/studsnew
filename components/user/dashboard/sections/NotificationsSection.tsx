...
function mapTypeToCategory(type: string): NotifCategory {
  ...
  if (t.includes("system") || t.includes("profile") || t.includes("info"))
    return "system";
  return "system";
}
          ...
          <div className="flex overflow-x-auto no-scrollbar gap-1 p-3">
            {categoryDefs.map((cat) => (
              ...
              >
                {cat.label}
                <span
                  className={`text-xs px-1.5 py-0.5 rounded-full ${cat.bgClass}`}
                >
                  {counts[cat.key] || 0}
                </span>
              </button>
          ...
          <div className="px-4 pb-3 flex justify-between items-center">
            <p className="text-xs text-gray-500">
              {currentTab === "all"
                ...
                : `Showing ${currentTab} notifications`}
            </p>
            <button
        ...
        {filteredNotifications.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100">
            {filteredNotifications.map((notif) => {
              const catDef =
                categoryDefs.find((c) => c.key === notif.type) ||
                categoryDefs[6];
              return (
                <div
                  key={notif.id}
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${catDef.iconBg}`}
                  >
                    <span
                      className={catDef.iconColor}
                      dangerouslySetInnerHTML={{
                        __html: getSvgForCategory(notif.type as NotifCategory),
                      }}
                    />
                  </div>
