    ...
    <div className="flex-1 bg-gray-50/50 min-h-screen">
      <div className="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-sm transition-all active:scale-95 whitespace-nowrap"
          >
            <Plus size={20} />
            Create News
          </button>
          ...
          <div className="flex flex-col xl:flex-row gap-4">
            <div className="flex-1 flex flex-wrap items-center gap-3">
              ...
              <select 
                ...
                className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500/20 outline-none cursor-pointer border-none"
              >
            ...
            <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-xl self-start">
              <button 
                ...
                className={`p-2 rounded-lg transition-all ${view === 'grid' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-900'}`}
              >
                <LayoutGrid size={18} />
              </button>
              <button 
                onClick={() => setView('table')}
                className={`p-2 rounded-lg transition-all ${view === 'table' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-900'}`}
              >
                <List size={18} />
              </button>
            </div>
        ...
        {selectedItems.size > 0 && (
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 mb-6 flex items-center justify-between animate-in fade-in slide-in-from-top-2">
            ...
            <div className="flex items-center gap-2">
              <button className="px-3 py-1.5 text-xs font-semibold bg-white border border-blue-200 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors">Publish</button>
              <button className="px-3 py-1.5 text-xs font-semibold bg-white border border-red-200 text-red-600 rounded-lg hover:bg-red-50 transition-colors">Delete</button>
            </div>
          </div>
        )}
              ...
              >
                ...
                <div className={`absolute top-4 right-4 z-10 opacity-0 group-hover:opacity-100 transition-opacity ${selectedItems.has(item.id) ? 'opacity-100' : ''}`}>
                  <input 
                    ...
                    className="w-5 h-5 rounded-md border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer bg-white shadow-sm"
                  />
                </div>
                ...
                {item.featured && (
                  <div className="absolute top-4 left-4 z-10">
                    <div className="flex items-center gap-1 bg-amber-500 text-white text-[10px] font-bold px-2 py-1 rounded-lg shadow-sm uppercase tracking-wider">
                ...
                <div className="relative h-48 overflow-hidden rounded-t-2xl">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                    <button 
                      ...
                      className="w-full bg-white/90 backdrop-blur-sm text-gray-900 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-white transition-colors"
                    >
                ...
                <div className="p-5">
                  <div className="flex items-center gap-2 mb-3">
                    <span className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-lg ${getCategoryStyle(item.category)}`}>
                      {item.category}
                    </span>
                    <span className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-lg border ${getStatusStyle(item.status)}`}>
                      {item.status}
                    </span>
                  </div>
      ...
      {isPreviewModalOpen && selectedNews && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
           ...
           <div className="relative bg-white w-full max-w-4xl max-h-[90vh] rounded-3xl overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
                <button 
                ...
                <div className="overflow-y-auto no-scrollbar">
                    <div className="relative h-72 sm:h-96 w-full">
                        ...
                        <div className="absolute bottom-10 left-10 right-10">
                            <span className={`px-3 py-1 text-[11px] font-bold uppercase tracking-wider rounded-lg mb-4 inline-block ${getCategoryStyle(selectedNews.category)} text-white`}>
                    ...
                    <div className="p-10">
                        ...
                        <div className="prose prose-blue max-w-none">
                            <p className="text-xl font-medium text-gray-900 mb-6 leading-relaxed bg-blue-50/50 p-6 rounded-2xl border-l-4 border-blue-500">
                                {selectedNews.desc}
                            </p>
                        </div>
                    </div>
            ...
            <div className="relative bg-white w-full max-w-6xl max-h-[92vh] rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col animate-in slide-in-from-bottom-5 duration-300">
                    ...
                    <div className="flex-1 p-10 space-y-8 border-r border-gray-100">
                        ...
                        <div className="space-y-4">
                            ...
                            <textarea 
                                ...
                                rows={3}
                                className="w-full p-6 bg-gray-50/50 border border-gray-200 rounded-2xl text-base text-gray-700 placeholder-gray-400 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all resize-none"
                            ></textarea>
                        </div>

                        <div className="space-y-4">
                            <label className="text-[11px] font-black text-gray-400 uppercase tracking-[0.2em]">Full Content</label>
                            <div className="border border-gray-200 rounded-[2rem] overflow-hidden bg-white shadow-sm ring-1 ring-black/5">
                                <div className="bg-gray-50/80 px-4 py-3 border-b border-gray-200 flex flex-wrap items-center gap-2">
                                    <button className="p-2 text-gray-600 hover:bg-white hover:shadow-sm rounded-xl transition-all">Type</button>
                                    <div className="w-px h-6 bg-gray-200 mx-1"></div>
                                    {['bold', 'italic', 'underline', 'link', 'image'].map(tool => (
                                        <button key={tool} className="p-2 text-gray-600 hover:bg-white hover:shadow-sm rounded-xl transition-all capitalize">{tool}</button>
                                    ))}
                                </div>
                                <div 
                    ...
                    <div className="w-full lg:w-[380px] bg-gray-50/50 p-10 space-y-8 flex-shrink-0">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="relative group cursor-pointer">
                                <div className="aspect-[4/3] bg-white border-2 border-dashed border-gray-200 rounded-3xl flex flex-col items-center justify-center p-8 transition-all group-hover:border-blue-500 group-hover:bg-blue-50/30">
                                    ...
                                    <button className="mt-6 px-4 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-600 shadow-sm group-hover:border-blue-200 group-hover:text-blue-600 transition-all">Browse Files</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-xl shadow-black/5 space-y-6">
                            <div className="space-y-3">
                                ...
                                <select className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-semibold text-gray-900 focus:ring-2 focus:ring-blue-500 outline-none">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-3">
                                    ...
                                    <input type="date" className="w-full bg-gray-50 border-none rounded-xl px-4 py-2.5 text-xs font-semibold text-gray-900" />
                                </div>
                                <div className="space-y-3">
                            ...
                            <label className="flex items-center gap-3 p-4 bg-amber-50 rounded-2xl border border-amber-100 cursor-pointer group hover:bg-amber-100 transition-colors">
                ...
                <div className="px-10 py-8 bg-white border-t border-gray-100 flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        <button className="px-8 py-3 text-sm font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-xl transition-all">
                            Save as Draft
                        </button>
