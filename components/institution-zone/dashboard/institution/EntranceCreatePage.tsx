...
const EntranceCreatePage = () => {
  ...
  const addQuestion = () => {
    setQuestions([...questions, { id: nextQuestionId++, text: "", type: "mcq", options: [""], marks: "" }]);
  };
  ...
  const handleSave = async (publish: boolean) => {
    ...
    try {
      ...
      if (editId) {
        ...
        if (publish) router.push("/institution-zone/dashboard/entrance/directory");
      } else {
        const res = await institutionEntranceApi.create(data);
        if (res?.id) {
          router.replace(`/institution-zone/dashboard/entrance/create?id=${res.id}`);
        }
      }
  ...
  if (loading) {
    return (
      <div className="p-4 md:p-6 lg:p-8">
        <SectionHeader title="Edit Entrance Exam" breadcrumbItems={[
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6">
        <SectionHeader title={editId ? "Edit Entrance Exam" : "Create Entrance Exam"} breadcrumbItems={[
          { label: "Dashboard", href: "/institution-zone/dashboard" },
          { label: "Entrance", href: "/institution-zone/dashboard/entrance" },
      ...
      <div className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Exam Date <span className="text-red-500">*</span></label>
            ...
            <div onClick={() => document.getElementById("bannerInput")?.click()}
              ...
              {heroBanner ? (
                ...
                <>
                  ...
                  <span className="mt-3 text-sm font-medium text-gray-900">Click to upload banner</span>
                  <span className="mt-1 text-xs text-gray-500">Recommended: 1920x600px</span>
                </>
              )}
              <input id="bannerInput" type="file" className="hidden" accept="image/*" onChange={e => {
      ...
      <div className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm mb-6">
        ...
        {questions.length === 0 && (
          <p className="text-sm text-gray-400 text-center py-8">No questions added yet. Click &quot;Add Question&quot; to start.</p>
        )}

        <div className="space-y-6">
          {questions.map((q, qIdx) => (
            <div key={q.id} className="border border-gray-200 rounded-xl p-5 relative">
              <div className="flex items-start justify-between mb-4">
                <span className="text-sm font-semibold text-gray-500">Question {qIdx + 1}</span>
                <button onClick={() => removeQuestion(q.id)} className="p-1.5 text-gray-400 hover:text-red-600 transition-colors" title="Remove question"><Trash /></button>
              </div>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="number" value={q.marks} onChange={e => updateQuestion(q.id, "marks", e.target.value)}
                      placeholder="e.g. 5"
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:border-blue-600 outline-none" />
                  </div>
                </div>
                    ...
                    <div className="space-y-2">
                      {q.options.map((opt, optIdx) => (
                        <div key={optIdx} className="flex items-center gap-2">
                          ...
                          {q.options.length > 1 && (
                            <button onClick={() => removeOption(q.id, optIdx)} className="p-1.5 text-gray-400 hover:text-red-600 transition-colors"><Trash /></button>
                          )}
                        </div>
                      ))}
                      <button onClick={() => addOption(q.id)} className="text-sm text-blue-600 font-medium hover:text-blue-700">+ Add Option</button>
                    </div>
                  </div>
                )}
              </div>
