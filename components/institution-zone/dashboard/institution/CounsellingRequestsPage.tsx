...
import { ChatsCircle, Clock, CheckCircle, MagnifyingGlass, CalendarPlus, X, Trash } from "@phosphor-icons/react";
import SectionHeader from "@/components/institution-zone/dashboard/shared/SectionHeader";
import { institutionCounsellingApi, CounsellingBooking, CounsellingSession } from "@/services/institutionCounsellingApi";
...
const CounsellingRequestsPage = () => {
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {b.status === "pending" && (
                    <div className="flex gap-1">
                      <button onClick={() => handleStatus(b.id, "confirmed")} className="p-1.5 hover:bg-green-50 rounded text-green-600" title="Confirm"><CheckCircle className="w-4 h-4" /></button>
      ...
      {showSlotModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) setShowSlotModal(false); }}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                <CalendarPlus className="text-blue-600" size={24} /> Schedule Session
              </h2>
              <button onClick={() => setShowSlotModal(false)} className="text-gray-400 hover:text-gray-600 transition-colors">
                <X size={24} />
              </button>
            </div>
            <div className="p-6">
                ...
                <div className="p-6 flex flex-col md:flex-row gap-8">
                  <div className="w-full md:w-1/3">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input type="text" value={slotTitle} onChange={e => setSlotTitle(e.target.value)}
                      ...
                      <div>
                        ...
                        <input type="number" value={slotCapacity} onChange={e => setSlotCapacity(e.target.value)}
                          min={1}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                      </div>
                      <button type="button" onClick={handleAddSlot} disabled={slotSubmitting || !slotDate || !slotTime}
                  ...
                  <div className="w-full md:w-2/3">
                    ...
                    <div className="bg-gray-50 rounded-md border border-gray-200 p-2 h-64 overflow-y-auto">
                      {sessions.length === 0 ? (
                        <p className="text-sm text-gray-400 text-center py-8">No slots created yet.</p>
                      ) : (
                        <ul className="space-y-2">
                          {sessions.map(s => (
                            <li key={s.id} className="flex justify-between items-center bg-white border border-gray-200 p-3 rounded-md hover:border-gray-300 transition-colors">
                              <div className="flex items-center text-sm font-medium text-gray-700">
                                <div className="w-10 h-10 rounded bg-indigo-50 text-indigo-600 flex flex-col items-center justify-center mr-3 leading-none">
                                  <span className="text-[10px] font-bold uppercase">{new Date(s.scheduled_at).toLocaleDateString("en-US", { month: "short" })}</span>
                                  <span className="text-sm font-black">{new Date(s.scheduled_at).getDate()}</span>
                                </div>
                                <div>
                                  <span className="block">{formatSlotDate(s)}</span>
                                  <span className="text-xs text-gray-500">{formatSlotTime(s)} &middot; {s.booked_seats}/{s.max_seats} booked</span>
                                </div>
                              </div>
            ...
            <div className="p-4 border-t border-gray-200 flex justify-end">
              <button onClick={() => setShowSlotModal(false)} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200 font-medium text-sm">Close</button>
            </div>
          </div>
        </div>
      )}
