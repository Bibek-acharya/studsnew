...
interface CurriculumItem { id: number; sem: string; sub: string; }
interface ScholarshipItem { id: number; type: string; elig: string; ben: string; }
interface PdfItem { id: number; name: string; link: string; }
interface FacilityItem { id: number; head: string; sub: string; }
interface FacultyItem { id: number; name: string; edu: string; exp: string; photo: string; }
...
const programMap: Record<StudyLevel, string[]> = {
  ...
  Bachelor: ["BIM", "BBA", "BCA", "BSc.CSIT", "BBM", "BBS", "BA", "BEd", "BE Civil", "BE Computer"],
...
const AdmissionManagePage: React.FC = () => {
  ...
  const clearFileInput = (input: HTMLInputElement | null, setter: React.Dispatch<React.SetStateAction<string>>) => {
        ...
        <form className="space-y-8" onSubmit={(event) => event.preventDefault()}>
          <div className={cardClass}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  {!coverPhoto && (
                    <div className="space-y-1 text-center relative z-10">
                      ...
                      <div className="flex text-sm text-gray-600 justify-center flex-wrap">
                        <button type="button" onClick={() => coverInputRef.current?.click()} className="font-medium text-blue-600 hover:text-blue-500">
                          Upload a file
                        </button>
              ...
              <div>
                ...
                <select value={programName} onChange={(event) => setProgramName(event.target.value)} className={inputClass}>
                  <option value="" disabled>{studyLevel ? "Select Program" : "Select Study Level First"}</option>
              ...
              <div>
                ...
                <input type="text" value={affiliation} onChange={(event) => setAffiliation(event.target.value)} placeholder="e.g., Tribhuvan University, NEB" className={inputClass} />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Duration</label>
                <input type="text" value={duration} onChange={(event) => setDuration(event.target.value)} placeholder="e.g., 4 Years (8 Semesters)" className={inputClass} />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Apply Now Link</label>
                <input type="url" value={applyLink} onChange={(event) => setApplyLink(event.target.value)} placeholder="e.g., https://apply.college.edu.np" className={inputClass} />
              </div>
            </div>
          </div>
          ...
          <div className={cardClass}>
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Overview / Summary</label>
                <textarea rows={3} value={overview} onChange={(event) => setOverview(event.target.value)} placeholder="Enter a brief introduction and summary of the course..." className={textareaClass} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Objectives</label>
                <textarea rows={3} value={objectives} onChange={(event) => setObjectives(event.target.value)} placeholder="List the primary objectives of this program..." className={textareaClass} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Eligibility Criteria</label>
                <textarea rows={2} value={eligibility} onChange={(event) => setEligibility(event.target.value)} placeholder="e.g., Minimum D+ grade in grade 11 and 12..." className={textareaClass} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Admission Process & Required Documents</label>
                <textarea rows={3} value={admissionProcess} onChange={(event) => setAdmissionProcess(event.target.value)} placeholder="Explain the step-by-step admission process and list required documents..." className={textareaClass} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Why Choose Us for this program?</label>
                <textarea rows={3} value={whyUs} onChange={(event) => setWhyUs(event.target.value)} placeholder="Highlight key features, labs, placement opportunities, etc..." className={textareaClass} />
              </div>
            </div>
          </div>
          ...
          <div className={cardClass}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <input type="number" value={feeTuition} onChange={(event) => setFeeTuition(event.target.value)} placeholder="0" className={inputClass} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Lab & Resources Fee</label>
                <input type="number" value={feeLab} onChange={(event) => setFeeLab(event.target.value)} placeholder="0" className={inputClass} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Examination Fee</label>
                <input type="number" value={feeExam} onChange={(event) => setFeeExam(event.target.value)} placeholder="0" className={inputClass} />
              </div>
              <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className={cardClass}>
              <div className="flex justify-between items-center border-b border-gray-200 pb-3 mb-4">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center"><BookOpen className="text-blue-500 w-6 mr-2" />Curriculum</h2>
                <button type="button" onClick={() => setCurriculum((prev) => [...prev, { id: nextId(prev), sem: "", sub: "" }])} className={addButtonClass}>+ Add Subject</button>
              </div>
            ...
            <div className={cardClass}>
              <div className="flex justify-between items-center border-b border-gray-200 pb-3 mb-4">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center"><GraduationCap className="text-blue-500 w-6 mr-2" />Scholarships</h2>
                <button type="button" onClick={() => setScholarships((prev) => [...prev, { id: nextId(prev), type: "", elig: "", ben: "" }])} className={addButtonClass}>+ Add Scholarship</button>
              </div>
              <div className="space-y-3">
                {scholarships.length === 0 ? (
                  ...
                  <div key={item.id} className="flex space-x-3 items-center bg-gray-50 p-2 rounded-md border border-gray-200">
                    ...
                    <button type="button" onClick={() => setScholarships((prev) => prev.filter((row) => row.id !== item.id))} className={removeButtonClass} title="Remove"><Trash2 className="w-4 h-4" /></button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className={cardClass}>
              <div className="flex justify-between items-center border-b border-gray-200 pb-3 mb-4">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center"><FileText className="text-blue-500 w-6 mr-2" />Model/Entrance PDFs</h2>
                <button type="button" onClick={() => setPdfs((prev) => [...prev, { id: nextId(prev), name: "", link: "" }])} className={addButtonClass}>+ Add PDF</button>
              </div>
              <div className="space-y-3">
                {pdfs.length === 0 ? (
            ...
            <div className={cardClass}>
              <div className="flex justify-between items-center border-b border-gray-200 pb-3 mb-4">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center"><Building2 className="text-blue-500 w-6 mr-2" />Facilities</h2>
                <button type="button" onClick={() => setFacilities((prev) => [...prev, { id: nextId(prev), head: "", sub: "" }])} className={addButtonClass}>+ Add Facility</button>
              </div>
              <div className="space-y-3">
                {facilities.length === 0 ? (
            ...
            <div className={`${cardClass} lg:col-span-2`}>
              <div className="flex justify-between items-center border-b border-gray-200 pb-3 mb-4">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center"><Presentation className="text-blue-500 w-6 mr-2" />Faculty Members</h2>
                <button type="button" onClick={() => setFaculty((prev) => [...prev, { id: nextId(prev), name: "", edu: "", exp: "", photo: "" }])} className={addButtonClass}>+ Add Faculty</button>
              </div>
              <div className="space-y-3">
                {faculty.length === 0 ? (
                    ...
                    <label className="w-12 h-12 flex-shrink-0 relative group rounded overflow-hidden border border-gray-300 bg-white flex items-center justify-center cursor-pointer">
                      ...
                      <input
                        ...
                        onChange={(event) => {
                          ...
                          setFaculty((prev) => prev.map((row) => row.id === item.id ? { ...row, photo: photoUrl } : row));
                        }}
                      />
                    </label>
                    <div className="flex-1 flex space-x-2">
                      <input type="text" value={item.name} onChange={(event) => setFaculty((prev) => prev.map((row) => row.id === item.id ? { ...row, name: event.target.value } : row))} placeholder="Full Name" className="w-1/3 border border-gray-300 p-2 rounded focus:ring-2 focus:ring-blue-500 outline-none text-sm bg-white transition" />
                      <input type="text" value={item.edu} onChange={(event) => setFaculty((prev) => prev.map((row) => row.id === item.id ? { ...row, edu: event.target.value } : row))} placeholder="Education Level" className="w-1/3 border border-gray-300 p-2 rounded focus:ring-2 focus:ring-blue-500 outline-none text-sm bg-white transition" />
                      <input type="text" value={item.exp} onChange={(event) => setFaculty((prev) => prev.map((row) => row.id === item.id ? { ...row, exp: event.target.value } : row))} placeholder="Experience" className="w-1/3 border border-gray-300 p-2 rounded focus:ring-2 focus:ring-blue-500 outline-none text-sm bg-white transition" />
                    </div>
                    <button type="button" onClick={() => setFaculty((prev) => prev.filter((row) => row.id !== item.id))} className={`${removeButtonClass} ml-2`} title="Remove"><Trash2 className="w-4 h-4" /></button>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className={`${cardClass} lg:col-span-2`}>
              <div className="flex justify-between items-center border-b border-gray-200 pb-3 mb-4">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center"><CircleHelp className="text-blue-500 w-6 mr-2" />FAQs</h2>
                <button type="button" onClick={() => setFaqs((prev) => [...prev, { id: nextId(prev), q: "", a: "" }])} className={addButtonClass}>+ Add FAQ</button>
              </div>
              <div className="space-y-3">
                {faqs.length === 0 ? (
                  ...
                  <div key={item.id} className="flex space-x-3 items-start bg-gray-50 p-3 rounded-md border border-gray-200">
                    ...
                    <button type="button" onClick={() => setFaqs((prev) => prev.filter((row) => row.id !== item.id))} className={`${removeButtonClass} mt-1`} title="Remove"><Trash2 className="w-4 h-4" /></button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className={cardClass}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              <div className="lg:col-span-2">
                ...
                <div className="flex items-center space-x-4">
                  <div className="h-16 w-16 rounded-full overflow-hidden bg-gray-100 border border-gray-300 flex-shrink-0 relative group">
                    {contactPhoto ? <img src={contactPhoto} alt="Contact preview" className="h-full w-full object-cover" /> : <div className="h-full w-full flex items-center justify-center text-gray-400"><Camera className="w-6 h-6" /></div>}
                    <button type="button" onClick={() => contactInputRef.current?.click()} className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><Camera className="text-white w-4 h-4" /></button>
                    <input ref={contactInputRef} type="file" className="hidden" accept="image/*" onChange={(event) => handleSingleImage(event, setContactPhoto)} />
                  </div>
                  <div className="flex flex-col">
                    <button type="button" onClick={() => contactInputRef.current?.click()} className="text-sm font-medium text-blue-600 hover:text-blue-500 text-left">Change Photo</button>
                    {contactPhoto && <button type="button" onClick={() => clearFileInput(contactInputRef.current, setContactPhoto)} className="text-xs text-red-500 hover:text-red-700 text-left mt-1">Remove</button>}
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <input type="text" value={contactName} onChange={(event) => setContactName(event.target.value)} placeholder="e.g., Dr. John Doe" className={inputClass} />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Position</label>
                <input type="text" value={contactPosition} onChange={(event) => setContactPosition(event.target.value)} placeholder="e.g., Head of Department" className={inputClass} />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Phone/Email</label>
                <input type="text" value={contactDetails} onChange={(event) => setContactDetails(event.target.value)} placeholder="e.g., 01-123456, info@college.edu" className={inputClass} />
              </div>
            </div>
          </div>
