...
const OrganizationProfile: React.FC = memo(() => {
  ...
  const handleFounderImageSelect = async (file: File) => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Failed to upload founder image",
      );
    } finally {
  ...
  const handleBrochureSelect = async (file: File) => {
    ...
    try {
      const url = await scholarshipProviderApi.uploadDocument(
        file,
        "brochures",
      );
      setBrochureUrl(url);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      setProfile(updated);
      if (updated.founder_name !== undefined)
        setFounderName(updated.founder_name);
      if (updated.founder_role !== undefined)
        setFounderRole(updated.founder_role);
      if (updated.founder_message !== undefined)
        setFounderMessage(updated.founder_message);
      ...
      if (updated.facebook_url !== undefined)
        setFacebookUrl(updated.facebook_url);
      if (updated.instagram_url !== undefined)
        setInstagramUrl(updated.instagram_url);
      if (updated.youtube_url !== undefined) setYoutubeUrl(updated.youtube_url);
      if (updated.linkedin_url !== undefined)
        setLinkedinUrl(updated.linkedin_url);
      if (updated.map_url !== undefined) setMapUrl(updated.map_url);
      if (updated.brochure_url !== undefined)
        setBrochureUrl(updated.brochure_url);
      if (updated.contact_number !== undefined)
  ...
  const handleContentSubmit = async (type: string, data: any) => {
    try {
      ...
      if (type === "services") {
        ...
        if (editing) {
          await updateSector(editing, data);
        } else {
        ...
        if (editing) {
          await updateGalleryImage(editing, data);
        } else {
  ...
  const handleContentDelete = async () => {
    ...
    try {
      ...
      if (t === "services") {
        ...
        await deleteSector(deleteModal.id);
      } else if (t === "projects") {
    ...
    <div className="space-y-6">
        ...
        <>
                ...
                {isEditing ? (
                  <>
                    <FileUpload
                      ...
                      previewClassName="w-[120px] h-[120px] object-contain rounded-lg border border-slate-200 bg-white"
                    />
                    {uploadingLogo && (
                  ...
                  <div className="relative overflow-hidden border border-slate-200 rounded-md">
                    {logoPreview ? (
                      ...
                      <div className="w-[120px] h-[120px] flex items-center justify-center text-gray-300 text-sm">
                        No logo
                      </div>
                    )}
                  </div>
                ...
                {isEditing ? (
                  <>
                    <FileUpload
                      ...
                      previewClassName="w-full h-[120px] object-cover rounded-lg border border-slate-200"
                      onClearPreview={handleBannerClear}
                    />
                    {uploadingBanner && (
                      <p className="mt-2 text-xs text-blue-600">
                        Uploading banner...
                  ...
                  <div className="relative overflow-hidden border border-slate-200 rounded-md">
                    {bannerPreview ? (
                      ...
                      <div className="w-full h-[120px] flex items-center justify-center text-gray-300 text-sm">
                        No banner uploaded
                      </div>
                    )}
                  </div>
          ...
          <div className="bg-white rounded-lg p-8 border border-slate-100">
            <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2 mb-6">
              <Building2 className="w-5 h-5 text-blue-600" /> Organization
              Details
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Field label="Email Address" value={email} isEditing={false}>
                <input
                  type="email"
              ...
              <Field label="PAN Number" value={panNumber} isEditing={isEditing}>
                <input
                  type="text"
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-500"
                  value={panNumber}
              ...
              <Field label="Mission" value={mission} isEditing={isEditing}>
                <textarea
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-500"
                  value={mission}
              ...
              <Field label="Values" value={values} isEditing={isEditing}>
                <textarea
                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-500"
                  value={values}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                >
                  <textarea
                    ...
                    rows={3}
                    placeholder="A short message or quote from the founder..."
                  />
                </Field>
              </div>
            </div>
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      PDF format preferred. Max size 5MB.
                    </p>
                    {brochureUrl ? (
                        ...
                        <a
                          href={
                            brochureUrl.startsWith("http")
                              ? brochureUrl
                              : `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}${brochureUrl.startsWith("/") ? "" : "/"}${brochureUrl}`
                          }
                          target="_blank"
                          rel="noreferrer"
      ...
      {activeTab === "Services" && (
        <ContentList
          ...
          items={services}
          onAdd={() =>
            setShowForm({ type: "services", data: {}, editing: null })
          }
          onEdit={(item: any) =>
            setShowForm({ type: "services", data: item, editing: item.id })
          }
          onDelete={(item: any) =>
            setDeleteModal({ isOpen: true, id: item.id, type: "services" })
          }
          renderItem={(item: any) => (
      ...
      {activeTab === "Sectors" && (
        <ContentList
          ...
          onEdit={(item: any) =>
            setShowForm({ type: "sectors", data: item, editing: item.id })
          }
          onDelete={(item: any) =>
            setDeleteModal({ isOpen: true, id: item.id, type: "sectors" })
          }
          renderItem={(item: any) => (
            <div className="flex items-center gap-3">
              {item.image_url ? (
      ...
      {activeTab === "Projects" && (
        <ContentList
          ...
          items={projects}
          onAdd={() =>
            setShowForm({ type: "projects", data: {}, editing: null })
          }
          onEdit={(item: any) =>
            setShowForm({ type: "projects", data: item, editing: item.id })
          }
          onDelete={(item: any) =>
            setDeleteModal({ isOpen: true, id: item.id, type: "projects" })
          }
          renderItem={(item: any) => (
            <div className="flex items-center gap-3">
              {item.image_url && (
          ...
          <div className="bg-white rounded-lg p-6 border border-slate-100">
            ...
            <FileUpload
              ...
              uploadedText={
                bulkUploading
                  ? "Uploading..."
                  : "Click to bulk upload gallery images"
              }
            />
            {bulkUploading && (
              <p className="mt-2 text-sm text-blue-600 animate-pulse">
                Processing bulk upload, please wait...
              </p>
            )}
          </div>
          ...
          <div className="space-y-8">
            ...
            {gallery.length === 0 && (
              <div className="bg-white rounded-lg p-12 border border-slate-100 text-center">
                ...
                <button
                  onClick={() =>
                    setShowForm({ type: "gallery", data: {}, editing: null })
                  }
  ...
  return (
    <div className="bg-white rounded-lg p-6 border border-slate-100 space-y-4 mb-6">
      {type === "services" && (
        <>
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1">
              <input
                placeholder="Icon Name (e.g. Coffee, Book, Globe, Heart)"
                value={form.icon || ""}
            ...
            <div className="w-10 h-10 rounded-lg border border-gray-200 flex items-center justify-center shrink-0 bg-white">
              {(() => {
                const name = form.icon
                  ? form.icon.charAt(0).toUpperCase() + form.icon.slice(1)
                  : "";
      ...
      {type === "gallery" && (
        <>
          <input
            placeholder="Folder Name (e.g. disaster, env)"
            value={form.folder || ""}
