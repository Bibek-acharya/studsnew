...
const quickCards = [
  {
    ...
    label: "Find Colleges",
    desc: "Get personalised college recommendations.",
    prompt:
      "Help me find the best colleges in Nepal for my studies. Ask me about my level and course interest.",
  },
  {
    icon: "fa-code",
    color: "text-[#0000ff]",
    label: "Explore Courses",
    desc: "Compare courses and career scope.",
    prompt:
      "I want to explore courses available in Nepal. What are the most in-demand fields and which programs should I consider?",
  },
  {
    icon: "fa-award",
    color: "text-purple-500",
    label: "Scholarships",
    desc: "Discover scholarships you qualify for.",
    prompt:
      "What scholarships are currently available on StudSphere? Help me find ones I might qualify for based on my profile.",
  },
  {
    icon: "fa-scale-balanced",
    color: "text-orange-500",
    label: "Compare",
    desc: "Side-by-side comparison of options.",
    prompt:
      "I want to compare colleges or courses. Walk me through what to consider and how StudSphere can help.",
  },
  {
    icon: "fa-headset",
    color: "text-cyan-500",
    label: "Counselling",
    desc: "Talk to a counsellor for guidance.",
    prompt:
      "I'd like to talk to a counsellor. What does StudSphere's counselling service offer and how do I book a session?",
  },
];
  ...
  return (
    <>
      <div className="flex h-screen w-full flex-col bg-slate-50">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`block h-1.5 w-1.5 rounded-full ${isStreaming ? "bg-amber-500 animate-pulse" : "bg-green-500"}`}
                ></span>
        ...
        <div
          ref={scrollRef}
          className="flex-1 overflow-y-auto custom-scroll bg-slate-50/50"
        >
          {!chatStarted ? (
            <div className="mx-auto flex w-full max-w-[1000px] flex-col items-center px-4 pt-[8vh] pb-8">
              <div className="mb-10 flex flex-col items-center">
                <div className="mb-2 flex items-center">
                  <h2 className="text-[22px] font-medium text-slate-700 md:text-[26px]">
                    Hi there 👋
                  </h2>
                </div>
                <h1 className="text-center text-[36px] font-medium tracking-tight text-slate-900 md:text-[44px]">
                  Where should we start?
                </h1>
                <p className="mt-3 text-center text-[14px] text-slate-500 max-w-md">
                  Ask anything about colleges, courses, exams, or scholarships
                  in Nepal. Powered by Sphere AI.
                </p>
              </div>
              <div className="flex w-full flex-wrap justify-center gap-4">
                {quickCards.map((card) => (
                  ...
                  >
                    <i
                      className={`fa-solid ${card.icon} ${card.color} text-[22px] transition-transform group-hover:scale-110 origin-left`}
                    ></i>
                    <div>
                      <div className="mb-1 text-[13px] font-semibold text-slate-800">
                        {card.label}
                      </div>
                      <div className="text-[11px] leading-[1.4] text-slate-400">
                        {card.desc}
                      </div>
                    </div>
                  </div>
              ...
              {messages.map((msg, i) =>
                msg.role === "user" ? (
                  <div key={i} className="flex w-full justify-end fade-in-up">
                    <div className="max-w-[85%] rounded-2xl rounded-tr-sm bg-brand-blue px-5 py-3.5 text-[15px] leading-relaxed text-white shadow-sm sm:max-w-[75%] whitespace-pre-wrap">
                      {msg.text}
                    </div>
                  </div>
                ) : (
                  <div
                    key={i}
                    className="flex w-full items-start gap-4 fade-in-up"
                  >
        ...
        <div className="border-t border-slate-200 bg-white p-3">
          <div className="mx-auto flex w-full max-w-[700px] flex-col gap-3">
            ...
            <div className="text-center text-[11px] text-slate-400">
              Sphere AI can make mistakes. Verify important details.
            </div>
          </div>
        </div>
      </div>
