  ...
  const districts = (NEPAL_DISTRICTS[provinceKey] || []).map(
    (districtName: string) => ({
      id:
        ...
        `d_${districtName.toLowerCase().replace(/[^a-z0-9]+/g, "_")}`,
      label: districtName,
    }),
  );
  return {
...
const SearchInput: React.FC<{
  ...
  <div className="relative mb-3">
    ...
    <input
      ...
      className="block w-full rounded-lg border border-gray-200 bg-[#f8fafc] py-2 pl-9 pr-3 text-[13.5px] text-gray-900 outline-none transition focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
    />
  </div>
);
    ...
    <div className="flex items-center gap-3">
      <div className="relative flex items-center justify-center">
        <input
          ...
          checked={checked}
          onChange={onChange}
          className="peer h-5 w-5 cursor-pointer appearance-none rounded border border-gray-300 bg-white transition-all checked:border-blue-600 checked:bg-blue-600"
        />
        <Check
          size={14}
  ...
  <label htmlFor={id} className="group flex cursor-pointer items-center gap-3">
    <div className="relative flex items-center justify-center">
      <input
        ...
        checked={checked}
        onChange={onChange}
        className="peer h-5 w-5 cursor-pointer appearance-none rounded-full border border-gray-300 bg-white transition-all checked:border-blue-600 checked:bg-blue-600"
      />
        ...
        {isQuickInquiryMode && (
          ...
          >
            <div className="relative flex h-6 w-6 items-center justify-center">
              ...
              <Check
                size={16}
                className="pointer-events-none absolute z-10 text-white opacity-0 transition-opacity peer-checked:opacity-100"
              />
            </div>
          </label>
        )}
      ...
      <div className="flex flex-1 flex-col px-0 pt-3">
        ...
        <div className="mb-2 flex min-w-0 items-center text-[14px] text-gray-500">
          ...
          <div className="flex min-w-0 flex-1 items-center gap-1.5">
            ...
            <span className="block min-w-0 truncate font-semibold text-slate-700">
        ...
        <div className="flex flex-col gap-3 mt-auto">
          <button
            type="button"
            onClick={() =>
              addToast(`Initiating counselling for ${college.name}...`)
            }
          ...
          <div className="flex gap-2">
            ...
            >
              <Bookmark
                className={`w-4 h-4 transition-all ${isSaved ? "text-blue-600 fill-blue-600" : "text-gray-400"}`}
              />
            </button>
          </div>
        </div>
      ...
      navigator.geolocation.getCurrentPosition(
        async (pos) => {
          try {
            ...
            addToast("Could not detect location.");
          } finally {
            setLocating(false);
          }
        },
        ...
        <aside className="w-full lg:w-[280px] shrink-0 bg-white rounded-3xl p-5 md:p-6 border border-gray-100 shadow-sm self-start">
          ...
          <div className="border-b border-gray-100 py-3">
            <button
              ...
              className="flex w-full items-center justify-start gap-2 rounded-md border border-gray-200 px-4 py-3 text-gray-700 hover:text-blue-600 outline-none transition-all duration-200 shadow-sm"
            >
              {locating ? (
              ...
              <span className="text-[15px] font-medium">
                {locating ? "Locating..." : "College Near Me"}
              </span>
            </button>
          </div>
            ...
            <div className="flex flex-col gap-3.5 pt-1">
              {ACADEMIC_LEVELS.map((item) => (
                <CheckboxItem
                  ...
                  onChange={() => toggleFilter("academic", item.id)}
                />
              ))}
            </div>
          </FilterAccordion>
              ...
              {filters.academic
                ...
                .map((item) => (
                  <CheckboxItem
                    ...
                    onChange={() => toggleFilter("program", item.id)}
                  />
                ))}
            </div>
          </FilterAccordion>
            ...
            <div className="flex flex-col gap-3.5 pt-1">
              {COLLEGE_TYPES.map((item) => (
                <CheckboxItem
                  ...
                  onChange={() => toggleFilter("type", item.id)}
                />
              ))}
            </div>
          </FilterAccordion>
          ...
          <div className="w-full flex flex-col gap-4 px-1">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="text-[16px] text-slate-900">
                Showing 1–{colleges.length} of {colleges.length}{" "}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <label className="flex items-center gap-2.5 cursor-pointer">
                <input
                  ...
                  className="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-600 accent-blue-600"
                />
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition ${quickApplyMode ? "translate-x-5" : "translate-x-0"}`}
                  />
                </button>
              </div>
            </div>
          ...
          <div className="w-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8">
            {collegesLoading
              ? Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={i}
                    className="max-w-[380px] w-full bg-white rounded-xl p-3.5 border border-gray-100 shadow-sm animate-pulse h-[400px]"
                  />
                ))
