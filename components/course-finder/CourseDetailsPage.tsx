...
const courseData: Record<string, {
  ...
  "1": { title: "10+2 Science", duration: "2 Years", level: "+2", field: "Science" },
  "2": { title: "10+2 Management", duration: "2 Years", level: "+2", field: "Management" },
  "3": { title: "BSc CSIT", duration: "4 Years", level: "Bachelor", field: "IT / Computer Science" },
  "4": { title: "BBA", duration: "4 Years", level: "Bachelor", field: "Management" },
  "5": { title: "MBA", duration: "2 Years", level: "Master", field: "Management" },
};
...
const CourseDetailsPage: React.FC<CourseDetailsPageProps> = ({ courseId, onBack, onNavigate }) => {
        ...
        <header className="banner-bg relative w-full min-h-[240px] md:min-h-[280px] rounded-2xl overflow-hidden flex items-center justify-center py-10 mb-12 shadow-sm">
          ...
          <div className="relative z-10 px-6 md:px-10 w-full flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
              ...
              <button 
                onClick={() => onNavigate("universitiesPage", { courseId, courseTitle, collegesCount: courseField })}
          ...
          <div className="lg:col-span-2 min-h-[60vh]">
            ...
            <section id="overview" className={`content-section ${activeTab === 'overview' ? 'block animate-fade-in' : 'hidden'}`}>
              ...
              <div className="prose prose-lg text-gray-600 space-y-6">
                <p>
                  The <strong className="text-gray-900 font-semibold">{courseTitle}</strong> program is meticulously designed to bridge the gap between secondary education and advanced university studies. Students will dive deep into fundamental scientific principles, mathematical logic, and practical applications.
                </p>
                <p>
                  This program is highly suitable for curious and analytical thinkers who want to shape the future of their chosen field. By the end of this course, you will be proficient in theoretical concepts and practical skills, preparing you for competitive entrance examinations.
                </p>
                <p>
                  The curriculum emphasizes hands-on learning, encouraging students to apply theoretical knowledge to real-world problems. By enrolling in top colleges with state-of-the-art facilities and expert faculty members, students receive an immersive educational experience that lays a robust foundation for a successful career.
                </p>
              </div>
            </section>
            ...
            <section id="eligibility" className={`content-section ${activeTab === 'eligibility' ? 'block animate-fade-in' : 'hidden'}`}>
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-6 mb-8">
                <p className="text-gray-800 font-medium mb-2">To be eligible for the {courseTitle} program, applicants must meet the following criteria:</p>
                <ul className="list-disc pl-5 text-gray-600 space-y-2 mt-4">
                  ...
                  <li>Must have secured at least a <strong className="text-gray-900">C+ grade</strong> in core subjects and a minimum of <strong className="text-gray-900">D+ grade</strong> in English and Nepali.</li>
                </ul>
              </div>
            </section>
            ...
            <section id="admission" className={`content-section ${activeTab === 'admission' ? 'block animate-fade-in' : 'hidden'}`}>
              ...
              <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-5 mb-8 text-gray-700 leading-relaxed">
                The admission is mainly open from <strong className="text-gray-900">July to August</strong> (shortly after SEE results), but the exact timeline is also heavily based on the individual college.
              </div>
              ...
              <div className="space-y-6">
                <div className="flex">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-gray-600 mt-2 leading-relaxed">Many colleges open online admission forms, while many others still require you to submit the form physically at the college.</p>
                  </div>
                </div>
                <div className="flex">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-gray-600 mt-2 leading-relaxed">Colleges can have different admission times and schedules for their entrance exams. You can find detailed entrance notices from the specific college page via <span className="text-blue-600 font-medium">Find College</span>.</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section id="courses" className={`content-section ${activeTab === 'courses' ? 'block animate-fade-in' : 'hidden'}`}>
              ...
              <p className="text-gray-600 mb-6">
                The program generally has two main sections: the <strong>Physical Group</strong> and the <strong>Biology Group</strong>. Below is the detailed breakdown of subjects and opportunities for each.
              </p>
            ...
            <section id="fee" className={`content-section ${activeTab === 'fee' ? 'block animate-fade-in' : 'hidden'}`}>
              ...
              <p className="text-gray-600 mb-6">Based on the college, here is the minimum and estimated fee structure in the tables below for a standard program.</p>
              ...
              <div className="space-y-4">
                <a href="#" className="bg-white border border-gray-200 p-5 rounded-xl flex flex-col sm:flex-row gap-5 hover:border-blue-600 hover:shadow-md transition-all group block">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mt-1.5 leading-relaxed">Various top colleges across the valley have started taking applications and entrance exams for the upcoming session.</p>
                  </div>
                </a>
                ...
                <a href="#" className="bg-white border border-gray-200 p-5 rounded-xl flex flex-col sm:flex-row gap-5 hover:border-blue-600 hover:shadow-md transition-all group block">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mt-1.5 leading-relaxed">The National Examination Board has officially published the SEE results. Check your GPA and find eligibility.</p>
                  </div>
                </a>
              </div>
            </section>
              ...
              <div className="mt-8">
                ...
                <div className="space-y-4 max-h-[calc(100vh-16rem)] overflow-y-auto pr-2 pb-4 custom-scrollbar">
                  {sponsoredColleges.map((college) => (
                    ...
                    >
                      <div className={`w-16 h-16 rounded-lg border border-gray-100 flex items-center justify-center shrink-0 p-1 ${college.logoFallbackColor || 'bg-white'}`}>
                        {college.logo ? (
                          <img src={college.logo} alt={`${college.name} Logo`} className="w-full h-full object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                        ) : (
                          <div className="flex flex-col items-center justify-center w-full h-full">
                            <span className="font-bold text-[11px] leading-tight text-center">{college.logoFallback}</span>
                          </div>
                        )}
                      </div>
                      <div className="flex flex-col flex-1 justify-center">
                        <div className="flex items-center gap-1.5">
                          <h4 className="font-bold text-gray-900 text-sm sm:text-base group-hover:text-blue-600 transition-colors line-clamp-1">{college.name}</h4>
                          {college.verified && <CheckCircle2 className="w-4 h-4 text-blue-600 shrink-0" />}
                        </div>
                        <div className="flex items-center mt-1 text-xs text-gray-500">
                          <MapPin className="w-3 h-3 mr-1 shrink-0" /><span className="truncate">{college.location}</span>
                        </div>
                        <div className="flex items-center mt-2 text-[11px] font-semibold text-blue-600 group-hover:text-blue-800 transition-colors">
                          <ExternalLink className="w-3 h-3 mr-1" /> {college.website}
                        </div>
                      </div>
                    </a>
                  ))}
