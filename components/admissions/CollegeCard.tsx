...
export default function CollegeCard({
  ...
  return (
    <div className="bg-white rounded-[20px] shadow-sm border border-gray-100 overflow-hidden w-full max-w-[340px] flex flex-col h-full transition-transform hover:shadow-md">
      {/* Image Section with Carousel */}
      <div className="p-2.5 pb-0 shrink-0">
        <div
          ref={carouselRef}
          className="group relative h-28 w-full bg-gray-200 rounded-[14px] overflow-hidden"
        >
          <div
          ...
          <div className="absolute bottom-2 right-3 flex items-center gap-1 z-10">
            {images.map((_, index) => (
              <button
                ...
                className={`dot-indicator transition-all duration-300 rounded-full shadow-sm focus:outline-none cursor-pointer ${
                  index === currentSlide
                    ? "w-3 h-1.5 bg-white"
      ...
      <div className="p-3 pb-3 flex flex-col flex-grow">
        ...
        {moreProgramsCount > 0 && (
          <a href="#" className="inline-flex items-center gap-1 text-[#2563eb] text-[12.5px] font-semibold hover:underline mb-2">
            ...
            <svg className="w-[10px] h-[10px] mt-0.5" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" />
            </svg>
          </a>
        )}
        ...
        <div className="flex items-center gap-1.5">
          <button
            onClick={onApply}
            className="flex-1 py-2 px-2 bg-[#2563eb] hover:bg-[#1d4ed8] text-white rounded-md text-[12px] font-bold transition-colors shadow-sm"
          >
            Apply Now
          </button>
          <button
