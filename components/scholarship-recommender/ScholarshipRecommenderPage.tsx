...
const stepTitles: Record<number, { title: string; subtitle: string }> = {
  ...
  2: {
    ...
    subtitle:
      "Help us find opportunities relevant to your intended field and preferences.",
  },
  3: {
    title: "Location",
    subtitle:
      "Many scholarships depend on province, district, and preferred study location.",
  },
  4: {
    title: "Tell us a bit more about yourself",
    subtitle:
      "This helps us include category and need-based opportunities you are eligible for.",
  },
  5: {
...
function StepShell({
  ...
  const contentWidthClass =
    step === 1 || step === 2
      ? "w-full max-w-137.5"
      : "max-w-md w-full mx-auto md:mx-0";
      ...
      <section className={rightWrapperClass}>
        <div className={contentWidthClass}>
          <header className="mb-8 text-center md:text-left">
            <h1 className="mb-3 text-[28px] font-bold leading-[1.2] tracking-tight text-slate-900 md:text-[36px]">
              {title}
            </h1>
...
interface Props {
  prefill?: Partial<RecommenderState>;
}

export default function ScholarshipRecommenderPage({ prefill }: Props) {
  const [step, setStep] = useState<StepIndex>(1);
  const [state, setState] = useState<RecommenderState>({
  ...
  const updateState = <K extends keyof RecommenderState>(
    key: K,
    value: RecommenderState[K],
  ) => {
    ...
    if (step === 5) {
      ...
      try {
        ...
        if (res.success && res.data?.scholarships) {
          const items: ScholarshipCardItem[] = res.data.scholarships.map(
            (s) => ({
              id: s.id,
              title: s.title,
          ...
          setError(
            "No matching scholarships found. Try adjusting your preferences.",
          );
        }
      } catch {
  ...
  const handleYesNoChange = (
    field: "willingEssay" | "willingInterview" | "willingGpa",
    value: "yes" | "no",
  ) => {
  ...
  const validateAcademicScore = (
    ...
    if (scoreType === "gpa" && numericScore > 4)
      return "GPA cannot be greater than 4.";
    if (scoreType === "percentage" && numericScore > 100)
      return "Percentage cannot be greater than 100.";
    return "";
  };

  const handleAcademicScoreTypeChange = (
    value: RecommenderState["academicScoreType"],
  ) => {
  ...
  const handleAcademicScoreChange = (value: string) => {
    ...
    setAcademicScoreError(
      validateAcademicScore(state.academicScoreType, value),
    );
  };
    ...
    <div className="flex gap-4.5 items-center mt-4">
      ...
      <button
        ...
        className={`inline-flex items-center justify-center h-12 rounded-md text-[18px] font-semibold text-white w-43.75 ${
          commonContinueEnabled
            ? "bg-brand-blue hover:bg-brand-hover shadow-[0_4px_14px_0_rgba(37,99,235,0.25)]"
            : "bg-[#afbacc]"
        }`}
      >
        Continue
          ...
          <div className="flex flex-col gap-2">
            ...
            <CustomSelect
              ...
              options={[
                {
                  value: "high_school",
                  label: "High School (10th/12th Grade)",
                },
                { value: "diploma", label: "Diploma / Certificate" },
                { value: "undergraduate", label: "Undergraduate (Bachelor's)" },
                {
                  value: "postgraduate",
                  label: "Postgraduate (Master's / PhD)",
                },
              ]}
            />
          </div>
              ...
              {[
                ...
                <button
                  ...
                  onClick={() =>
                    updateState(
                      "studyMode",
                      value as RecommenderState["studyMode"],
                    )
                  }
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div className="relative sm:col-span-1">
                <CustomSelect
                  ...
                  onChange={(value) =>
                    handleAcademicScoreTypeChange(
                      value as RecommenderState["academicScoreType"],
                    )
                  }
                  options={[
                    { value: "gpa", label: "GPA" },
              ...
              <input
                ...
                onChange={(event) =>
                  handleAcademicScoreChange(event.target.value)
                }
                placeholder={
      ...
      {step === 2 && (
        <form className="flex flex-col gap-8 mb-10">
          ...
          <div className="space-y-6">
            <h3 className="text-[18px] font-bold text-[#0f172a] border-b border-slate-100 pb-2">
              Are you willing to:
            </h3>
                  ...
                  {[
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleYesNoChange(
                          field as
                            "willingEssay" | "willingInterview" | "willingGpa",
                          value as "yes" | "no",
                        )
                      }
            ...
            <CustomSelect
              ...
              options={
                state.province
                  ? (
                      NEPAL_DISTRICTS[
                        state.province as keyof typeof NEPAL_DISTRICTS
                      ] || []
              ...
              {[
                ...
                <button
                  ...
                  onClick={() =>
                    updateState(
                      "studyLocation",
                      value as RecommenderState["studyLocation"],
                    )
                  }
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <button
                  ...
                  type="button"
                  onClick={() =>
                    updateState("income", value as RecommenderState["income"])
                  }
        ...
        <form className="space-y-6">
          <MultiSelect
            ...
            options={[
              ...
              {
                value: "Arts / Creative Writing",
                label: "Arts / Creative Writing",
              },
            ...
            options={[
              {
                ...
                label: "Academic Excellence Awards",
              },
              {
                value: "National Level Sports",
                label: "National Level Sports",
              },
              {
                value: "Student Leadership Award",
                label: "Student Leadership Award",
              },
              {
                value: "Science / Math Olympiad",
                label: "Science / Math Olympiad",
              },
