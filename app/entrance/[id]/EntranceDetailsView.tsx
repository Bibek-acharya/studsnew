...
/**
 * Normalizes the stored map value for rendering.
 * Admins may paste either full iframe embed code or just the embed URL
 * (e.g. "https://www.google.com/maps/embed?pb=..."). A bare URL must be
 * wrapped in an iframe, otherwise RichText renders it as plain text/link.
 */
const richMapHtml = (value: string) => {
  const trimmed = value.trim();
  if (/<iframe[\s>]/i.test(trimmed)) return trimmed;
  const urlMatch = trimmed.match(/^https?:\/\/\S+$/i);
  const src = urlMatch
    ? trimmed
    : trimmed.match(/src=["']([^"']+)["']/i)?.[1];
  if (!src) return trimmed;
  const escaped = src.replace(/"/g, "&quot;");
  return `<iframe src="${escaped}" width="100%" height="200" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>`;
};
...
function mapExamToDetails(exam: Exam): ExamDetails {
  ...
  const getPatternValue = (...labelKeywords: string[]): string => {
    if (!exam.examPattern) return "";
    for (const item of exam.examPattern) {
      if (
        labelKeywords.some((kw) =>
          item.label.toLowerCase().includes(kw.toLowerCase()),
        )
      ) {
        return item.value;
      }
  ...
  return {
    ...
    embeddedMap: exam.embeddedMap || "",
  };
}

const EntranceDetailsPage: React.FC<{
  id: string;
  initialData: EntranceDetailsResponse | null;
}> = ({ id, initialData }) => {
  const router = useRouter();
  ...
  const { data: apiData, isLoading } = useQuery({
    ...
    // Server-fetched initial data (fresh for staleTime, but marked stale via
    // initialDataUpdatedAt 0 so logged-in institution owners still get their
    // instData refetch exactly as before).
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="mx-auto max-w-350 px-4 sm:px-6 lg:px-8 py-8 md:py-12 grid grid-cols-1 lg:grid-cols-3 gap-10 md:gap-10 bg-white">
        ...
        <div className="lg:col-span-2 min-h-[500px]">
                    ...
                    {exam.modelSets.map((set, idx) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="text-[#0000ff] hover:bg-blue-50 p-2 rounded-full transition-colors"
                        >
              ...
              {exam.requiredDocuments && exam.requiredDocuments.length > 0 ? (
                <div>
                  <div className="mb-8">
                    ...
                    <p className="text-gray-600">
                      List of documents required for {exam.title} application.
                    </p>
                  </div>
                  <div className="space-y-4">
                ...
                <EmptyTabState tabName="Required Documents" />
              )}
            </div>
          )}
          ...
          {activeTab === "schedule" && (
            <div>
              {exam.examinationSchedule &&
              exam.examinationSchedule.length > 0 ? (
                <div>
                  <div className="mb-8">
                    <h2 className="text-[22px] font-bold text-gray-900 mb-2">
                      Exam Schedule
                    ...
                    <p className="text-gray-600">
                      Detailed examination schedule for {exam.title}.
                    </p>
                  </div>
                  <div className="overflow-x-auto rounded-md border border-gray-200">
                    <table className="w-full text-left border-collapse min-w-[600px]">
                      ...
                      <tbody className="text-[15px]">
                        {exam.examinationSchedule.map((item, idx) => (
                          <tr
                            key={idx}
                            className="border-b border-gray-200 hover:bg-gray-50"
                          >
                ...
                <EmptyTabState tabName="Exam Schedule" />
              )}
            </div>
          )}
          ...
          {activeTab === "programs" && (
            <div>
              {exam.programsOffered && exam.programsOffered.length > 0 ? (
                <div>
                  <div className="mb-8">
                    <h2 className="text-[22px] font-bold text-gray-900 mb-2">
                      Programs Offered
                    ...
                    <p className="text-gray-600">
                      Programs available under {exam.title}.
                    </p>
                  </div>
                  <div className="space-y-4">
                    {exam.programsOffered.map((program, idx) => (
                      <div
                        key={idx}
                        className="border border-gray-200 rounded-md p-5 bg-white"
                      >
                ...
                <EmptyTabState tabName="Programs Offered" />
              )}
            </div>
          )}
        </div>
            ...
            <div className="bg-white border border-gray-100 rounded-md p-5">
              ...
              <ul className="space-y-4">
                ...
                {exam.socialLinks &&
                  ...
                  exam.socialLinks.map((link, idx) => {
                    ...
                    const socialIcon = platformIcons[link.platform] || (
                      <Globe className="w-4 h-4" />
                    );
                    return (
                      <li
              ...
              {exam.embeddedMap && (
                <div className="mt-5 pt-5 border-t border-gray-100">
                  <RichText
                    html={richMapHtml(exam.embeddedMap)}
                    allowIframe
