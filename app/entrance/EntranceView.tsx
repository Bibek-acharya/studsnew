...
import type { EntrancesResponse } from "@/services/entrance.api";

interface EntranceViewProps {
  initialData?: EntrancesResponse;
}

const EntranceView: React.FC<EntranceViewProps> = ({ initialData }) => {
    ...
    <div className="min-h-screen p-4 text-gray-800 md:p-6 lg:p-8">
      <div className="mx-auto flex max-w-350 flex-col gap-6 lg:flex-row lg:flex-nowrap lg:gap-8">
        ...
        <main className="min-w-0 flex-1">
          <EntranceGrid initialData={initialData} filters={filters} setFilters={setFilters} onMobileFilterClick={() => setShowMobileFilters(true)} />
        </main>
      </div>
    </div>
  );
