...
const cardData = {
  "Samir Sharma": {
    avatar: "https://images.unsplash.com/photo-1480455624313-e29b44bbfde1?auto=format&fit=crop&w=150&h=150&q=80",
    title: "Industry-Aligned<br/>Curriculum",
    quote: "\"Our program bridges the gap between theoretical foundations and the practical skills needed in today's rapidly evolving tech landscape.\"",
  ...
  "Deepak Khadka": {
    ...
    quote: "\"We ensure a smooth, transparent, and welcoming enrollment process for every prospective student joining our community.\"",
  ...
  "Basanta Blown": {
    ...
    quote: "\"Guiding students through their academic journey with personalized assistance, care, and continuous mentorship.\"",
  ...
  "Tribendra Timsina": {
    ...
    quote: "\"Maintaining rigorous standards in our curriculum to foster critical thinking, innovation, and professional growth.\"",
  ...
  "Kush Shrestha": {
    ...
    quote: "\"Behind every great institution is a dedicated administrative team ensuring smooth and efficient day-to-day operations.\"",
    ...
    <div className="mx-auto mb-10 flex w-full max-w-212.5 flex-col items-center justify-center gap-6 xl:flex-row xl:gap-8">
      <div className="relative h-[50vh] w-full max-w-125 shrink-0 overflow-hidden rounded-xl bg-gray-900 shadow-2xl ring-1 ring-gray-200/50 sm:h-[340px] sm:rounded-2xl">
        ...
        <div className="absolute bottom-5 left-5 z-10 max-w-[70%]">
          <div className="flex flex-col rounded-lg border border-white/10 bg-black/60 px-4 py-2 text-white shadow-lg backdrop-blur-md sm:px-5 sm:py-3" style={{textShadow: "0 1px 2px rgba(0,0,0,0.8)"}}>
            <span className="text-sm font-semibold tracking-wide sm:text-base">{mainKey}</span>
            <span className="mt-0.5 text-[10px] font-medium text-gray-300 sm:text-xs">{mainData.role}</span>
          </div>
        </div>
        ...
        <div className="absolute right-5 top-5 z-20 flex max-h-[calc(100%-40px)] flex-col gap-2 overflow-y-auto pb-4 pr-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {others.map((key) => {
            ...
            return (
              <div key={key} onClick={() => handleSwap(key)} className="group relative h-[50px] w-[70px] shrink-0 cursor-pointer transition-transform hover:scale-105 sm:h-[55px] sm:w-[85px] shadow-xl">
                <div className="relative h-full w-full overflow-hidden rounded-lg border-[2px] border-white bg-gray-800 sm:rounded-xl">
                  <video 
                    ...
                    src={data.video} autoPlay loop muted playsInline
                  />
                  <div className="absolute bottom-0 left-0 right-0 z-30 flex h-[80%] flex-col justify-end bg-gradient-to-t from-black/90 via-black/40 to-transparent p-1">
                    <span className="truncate text-[8px] font-bold leading-tight text-white sm:text-[9px]" style={{textShadow: "0 1px 2px rgba(0,0,0,0.8)"}}>{key}</span>
                    <span className="mt-0.5 truncate text-[6px] font-medium leading-tight text-gray-300 sm:text-[7px]" style={{textShadow: "0 1px 2px rgba(0,0,0,0.8)"}}>{data.role}</span>
                  </div>
                </div>
              </div>
            );
...
const downloads = [
  { title: "Prospectus 2025", size: "PDF, 8.2 MB", color: "bg-[#0000FF]/5 text-[#0000FF]", btn: "bg-[#0000FF] hover:bg-[#0000CC]" },
  {
...
const CollegeDetailsPage: React.FC = () => {
  ...
  useEffect(() => {
    if (collegeId) {
      setLoading(true);
      apiService.getCollegeById(collegeId).then(res => {
        setCollege(res.data);
        setLoading(false);
      ...
      <div className="grid grid-cols-1 gap-10 bg-[#f8fafc] px-6 py-8 md:gap-14 md:px-12 md:py-12 lg:grid-cols-3 lg:px-24 xl:px-32">
        <div className="lg:col-span-2">
          ...
          {activeTab === "courses" && (
            <div className="overflow-hidden rounded-[20px] border border-gray-100 bg-white shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 bg-[#f4f8fc] px-6 py-4">
                <p className="text-[14px] font-semibold text-[#0000FF]">Fees in NPR/year – filter by level</p>
                  ...
                  {filteredCourses.map((course) => (
                    <div key={course.name} className="grid grid-cols-12 items-center gap-4 border-b border-gray-100 px-6 py-5 hover:bg-gray-50/50">
                      ...
                      <div className="col-span-3">
                        <h4 className="text-[15.5px] font-bold text-[#0000FF]">{course.fees}</h4>
          ...
          {activeTab === "admissions" && (
            <div className="rounded-[20px] border border-gray-100 bg-white p-5 shadow-sm">
              <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                <p className="text-[14px] font-semibold text-[#0000FF]">Admission notices – filter by level</p>
          ...
          {activeTab === "offered" && (
            <div className="overflow-hidden rounded-[20px] border border-gray-100 bg-white shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 bg-[#f4f8fc] px-6 py-4">
                <p className="text-[14px] font-semibold text-[#0000FF]">Programs offered – filter by level</p>
                ...
                <div className="min-w-[800px]">
                  ...
                  {filteredPrograms.map((program) => (
                    <div key={program.name} className="grid grid-cols-12 items-center gap-4 border-b border-gray-100 px-6 py-5 hover:bg-gray-50/50">
                      ...
                      <div className="col-span-2">
                        <button className="rounded-lg bg-[#0000FF]/5 px-4 py-2 text-xs font-bold text-[#0000FF] hover:bg-[#0000FF]/10">View Details</button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === "scholarship" && (
            <div className="overflow-hidden rounded-[20px] border border-gray-100 bg-white shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 bg-[#f4f8fc] px-6 py-4">
                <p className="text-[14px] font-semibold text-[#0000FF]">Scholarship opportunities – filter by level</p>
                ...
                <div className="min-w-[800px]">
                  ...
                  {filteredScholarships.map((scholarship) => (
                    ...
                    >
                      ...
                      <div className="col-span-3">
                        <button className="rounded-lg bg-[#0000FF] px-5 py-2 text-xs font-bold text-white hover:bg-[#0000CC]">Get Scholarship</button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === "gallery" && (
            <div>
              <div className="mb-6 flex items-center justify-between">
                ...
                <button className="text-[13.5px] font-bold text-[#0000FF]">View All</button>
              </div>
...
const ContactInfoRow: React.FC<{
  ...
  <li className="flex items-start gap-3">
    ...
    <div>
      ...
      {link ? (
        <a href="#" className="mt-0.5 block text-[13.5px] text-[#0000FF] hover:underline">
          {value}
        </a>
      ) : (
        ...
        <div className="overflow-y-auto px-6 py-5">
          <div className="mb-5 flex items-start gap-3 rounded-lg border border-[#0000FF]/20 bg-[#0000FF]/5 p-3.5">
            ...
            <p className="line-height-extra text-[13px] text-[#0000FF]">
              Provide official details to claim <span className="font-bold text-[#0000FF]">{collegeName}</span>. Upon verification, you will receive full control over this profile.
            </p>
          </div>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="mt-8 flex flex-col justify-end gap-3 sm:flex-row pt-4">
              ...
              <button type="submit" className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#0000FF] px-6 py-2.5 text-[14px] font-bold text-white shadow-[0_4px_12px_rgba(0,0,255,0.2)] transition-all hover:-translate-y-0.5 hover:bg-[#0000CC] sm:w-auto">
