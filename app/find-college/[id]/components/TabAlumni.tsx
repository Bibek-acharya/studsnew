      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        {alumni.map((person: any, i: number) => (
          <div key={person.name || i} className="flex items-center gap-4 rounded-md border border-gray-200 bg-white p-5">
            ...
            {person.linkedin && (
              <a href={person.linkedin.startsWith("http") ? person.linkedin : `https://${person.linkedin}`} target="_blank" rel="noreferrer" className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-blue/5 text-brand-blue hover:bg-brand-blue/10"><i className="fa-brands fa-linkedin-in"></i></a>
            )}
          </div>
        ))}
      </div>
