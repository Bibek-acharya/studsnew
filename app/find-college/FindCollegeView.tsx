...
interface FindCollegeViewProps {
  initialData?: {
    data: {
      colleges: College[];
...
const FindCollegeView: React.FC<FindCollegeViewProps> = ({ initialData }) => {
  const router = useRouter();
  const [filters, setFilters] = useState<CollegeFilters>(DEFAULT_COLLEGE_FILTERS);
  const [showMobileFilters, setShowMobileFilters] = useState(false);

  const handleNavigate = (view: string, data?: { id?: string | number; tab?: string }) => {
...
export default FindCollegeView;
